...
void main() {
  ...
  test('the duplicate-shelf refusal reaches the screen, index and all', () {
    ...
    final state = refusedWith({
      'items.1.stock_item_id': ['لا يمكن تكرار نفس الصنف أكثر من مرة في أمر الشراء'],
    });
    ...
    expect(message, 'لا يمكن تكرار نفس الصنف أكثر من مرة في أمر الشراء');
  });
  ...
  test('a complaint about the list as a whole still wins over an indexed one', () {
    ...
    final state = refusedWith({
      ...
      'items.0.stock_item_id': ['الصنف المخزني المحدد غير موجود'],
    });
