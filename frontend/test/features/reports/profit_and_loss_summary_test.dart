...
void main() {
  ...
  group('what the screen asks the model', () {
    ...
    test('the margin is the share of the revenue that survived the cost', () {
      ...
      // Act
      final label = summary.grossMarginLabel;

      // Assert — 58.82%, and a percent is read whole
      expect(summary.grossMarginPercent, 59);
      expect(label, '59%');
    });

    test('a period that earned nothing has no margin to state', () {
      // Arrange — dividing by a revenue of zero is not a 0% margin, it is no answer at all
      const summary = ProfitAndLossSummary(
      ...
      // Act & Assert
      expect(summary.grossMarginPercent, isNull);
      expect(summary.grossMarginLabel, isNull);
    });

    test('a figure that cannot be read has no margin either', () {
      // Arrange — the same rule `isLoss` follows: a number nobody could parse is not answered
      // with a made-up one
      const odd = ProfitAndLossSummary(
        period: PnlPeriod(from: '2026-03-01', to: '2026-03-31'),
        revenue: PnlRevenue(product: '75.00', service: '0.00', total: '75.00'),
      ...
      // Act & Assert
      expect(odd.grossMarginPercent, isNull);
    });

    test('a losing period states a negative margin, and lets the screen decide', () {
      // Arrange — -45 out of 75 earned
      const losing = ProfitAndLossSummary(
        period: PnlPeriod(from: '2026-03-01', to: '2026-03-31'),
        revenue: PnlRevenue(product: '75.00', service: '0.00', total: '75.00'),
        costOfGoodsSold: PnlCostOfGoodsSold(
      ...
      // Act & Assert — the model answers the arithmetic; the donut is what the screen refuses
      expect(losing.grossMarginPercent, -60);
    });
