# دعاية (Dayaa) — Flutter Rules

> المعيار الملزم لكيفية بناء تطبيق **دعاية**.
...
## ⛔ قاعدة فوق كل القواعد — لا جهاز، أبداً

**لا يُشغَّل التطبيق ولا يُلمس أيُّ جهاز — لا هاتفٌ موصول، ولا محاكي iOS (Simulator)، ولا
محاكي أندرويد (Emulator). أبداً.** قرارُ المالك 2026-09-24: «Never Test Or Touch the Device
(Simulator Or Emulator) — Never ever».

- ممنوع: `flutter run`، و`flutter drive`، و`flutter install`، و`flutter attach`، وفتحُ
  المحاكي أو إقلاعُه، و`xcrun simctl`، و`adb`، وإعادةُ التحميل السريع، وكلُّ أداةٍ تقود التطبيق
  وهو يعمل (`hot_reload`، `flutter_driver`، `widget_inspector`).
- **المالكُ وحده يجرّب على الجهاز.** التحقّق من هنا يقف عند ما لا يحتاج جهازاً، ويُسلَّم ما
  بقي للتجربة على الهاتف مكتوباً.
...
dart format -l 100 <الملفات>                 # **دائماً بـ -l 100** — انظر أدناه
flutter analyze                              # يجب أن يخرج "No issues found!"
flutter test                                 # كل الاختبارات
```

> **`dart format` المجرَّد يعيد لفّ الملف على ٨٠ عموداً، وهذا المشروع مكتوب على ١٠٠.** تشغيله بلا
> `-l 100` يُنتج تعديلاً ضخماً في ملفات لم يمسّها أحد، فيغرق التغيير الحقيقي في ضجيج. ولا تُشغّله
> على المستودع كله: بعض الملفات القديمة ليست نظيفة حتى على ١٠٠، فتُترك كما هي وتُنسَّق الملفات
> التي عُدِّلت وحدها.
...
- **الرسائل للمستخدم بالعربية، والتعليقات بالعربية** — التعليق يُكتب لمن يقرأ الشيفرة هنا. الكود نفسه (الأسماء والأنواع) بالإنجليزية.
...
| [`DismissKeyboard`](lib/core/widgets/dismiss_keyboard.dart) | **مركّب مسبقاً في [app.dart](lib/app.dart)** — لا تستعمله في شاشة |
...
- **النقر خارج أي حقل يُغلق لوحة المفاتيح**، وهذا سلوك التطبيق لا سلوك شاشة. مركّب مرة واحدة في [app.dart](lib/app.dart) عبر [`DismissKeyboard`](lib/core/widgets/dismiss_keyboard.dart) — لا تكرّره في شاشة، ولا تعتمد على شاشة أن تتذكّره.
- **طبقتان لأنهما تغطّيان حالتين مختلفتين:** الغلاف يلتقط النقر على الفراغ، و`onTapOutside` في [`AppTextField`](lib/core/widgets/app_text_field.dart) يلتقط النقر الذي **يخطفه ويدجت آخر** (زر، صف قائمة) — لأن الأعمق يفوز بحلبة الإيماءات فلا يصل النقر إلى الغلاف، بينما `TapRegion` ليس في الحلبة أصلاً.
- **السحب ليس نقراً.** لا تُغلق لوحة المفاتيح عند التمرير: الوصول إلى الحقل التالي في نموذج طويل يمرّ بالتمرير، وإغلاقها في الطريق يجعل النموذج غير قابل للاستعمال.
- **سلوك Flutter الافتراضي لا يكفي**: `onTapOutside` الافتراضي يُغلق على سطح المكتب والويب فقط، ويُبقي التركيز عمداً على الهاتف.
