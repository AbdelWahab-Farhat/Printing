...
@JsonKey(name: 'is_closed') bool get isClosed;/// «مستعجلة» — what the customer asked for, not how long the order has waited.
///
...
@JsonKey(name: 'is_urgent') bool get isUrgent;/// Whether «رسالة الجاهزية» is a question this order has reached at all.
///
...
@JsonKey(name: 'ready_message_applies') bool get readyMessageApplies;/// Whether somebody has said the customer was told.
///
...
@JsonKey(name: 'is_ready_message_sent') bool get isReadyMessageSent;@JsonKey(name: 'ready_message_sent_at') DateTime? get readyMessageSentAt;/// Who said so. **Null on a list row** — the server sends the name with the full order only,
/// because no card shows it and a page of twenty would be a query per row.
@JsonKey(name: 'ready_message_sent_by') OrderActor? get readyMessageSentBy;// ── العربون ──────────────────────────────────────────────────────────────────────────
//
...
@JsonKey(name: 'deposit_expected_amount') String? get depositExpectedAmount;@JsonKey(name: 'deposit_expected_method') String? get depositExpectedMethod;@JsonKey(name: 'deposit_expected_method_label') String? get depositExpectedMethodLabel;/// When the order was *said* to have been paid. Cleared if the claim is walked back.
@JsonKey(name: 'deposit_paid_at') DateTime? get depositPaidAt;/// An employee's tick that the money really landed — **a statement, not arithmetic.** It
...
@JsonKey(name: 'is_deposit_received') bool get isDepositReceived;@JsonKey(name: 'deposit_confirmed_at') DateTime? get depositConfirmedAt;/// Whether **this** reader may tick it — and the only thing the switch is ever gated on.
///
...
@JsonKey(name: 'can_confirm_deposit') bool get canConfirmDeposit;/// A deposit declared paid that nobody has confirmed — the accountant's queue.
@JsonKey(name: 'awaits_deposit_confirmation') bool get awaitsDepositConfirmation;/// Who said it was paid, and who confirmed it. **Null on a list row** — the server sends
/// the names with the full order only, exactly as it does for [readyMessageSentBy].
@JsonKey(name: 'deposit_claimed_by') OrderActor? get depositClaimedBy;@JsonKey(name: 'deposit_confirmed_by') OrderActor? get depositConfirmedBy;/// Whether the customer left part of this order behind.
///
...
/// «لا».
@JsonKey(name: 'is_partially_delivered') bool? get isPartiallyDelivered;/// The moves this order may make, **already narrowed to what the signed-in user may do.**
...
@JsonKey(name: 'written_off_amount') String get writtenOffAmount;/// What the customer paid **to the courier** for delivery, remitted to the carrier rather
/// than to us.
///
...
@JsonKey(name: 'carrier_settled_amount') String get carrierSettledAmount;/// What is still owed — the invoice less what was collected **and** what was forgiven.
...
@JsonKey(name: 'nawris_parcel') NawrisParcelRef? get nawrisParcel;/// The number the man holding the parcel can be reached on — what «جاري التوصيل» asks for
...
@JsonKey(name: 'courier_phone') String? get courierPhone;@JsonKey(name: 'cancellation_reason') String? get cancellationReason;/// Where «تراجع عن الإلغاء» would put this order back, or null when the undo is not on
...
@JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown) OrderStatus? get reinstateTo;/// The Arabic the server chose for [reinstateTo]. Rendered as-is, for the reason
/// [statusLabel] gives: a status this build has never heard of still reads correctly.
@JsonKey(name: 'reinstate_to_label') String? get reinstateToLabel;/// Whether «تراجع عن التسوية» is on offer — settled, and this user holds the grant. A flag
/// rather than a status: the undo always lands on «تم الاستلام».
@JsonKey(name: 'can_unsettle') bool get canUnsettle;/// Where «تراجع عن التسليم» would put this order back, or null when it is not on offer — not
...
@JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown) OrderStatus? get undoDeliveryTo;/// The Arabic the server chose for [undoDeliveryTo], rendered as-is.
@JsonKey(name: 'undo_delivery_to_label') String? get undoDeliveryToLabel;/// The journey, in the domain's own order — see [OrderProgress].
...
@JsonKey(name: 'fulfillment_warehouse_id') int? get fulfillmentWarehouseId;@JsonKey(name: 'stock_deducted_at') DateTime? get stockDeductedAt;/// Who is making it, for an order a vendor executes. Null on everything we make ourselves.
@JsonKey(name: 'vendor_id') int? get vendorId;/// The vendor's name **as this order said it**, snapshotted the way [cityName] is. A vendor
/// renamed since keeps its new name everywhere except here, which is the point of storing
/// it — show this, never a lookup.
@JsonKey(name: 'vendor_name') String? get vendorName;/// When the job went out to the vendor. Null until it does, and forever on an order that
/// never walks the وسيط road.
@JsonKey(name: 'manufacturing_started_at') DateTime? get manufacturingStartedAt;/// What the parcel weighs, in kilograms — the lines' own scale readings added up by the
/// server.
///
...
@JsonKey(name: 'total_weight') String? get totalWeight;@JsonKey(name: 'placed_at') DateTime? get placedAt;@JsonKey(name: 'delivered_at') DateTime? get deliveredAt;@JsonKey(name: 'settled_at') DateTime? get settledAt;@JsonKey(name: 'created_at') DateTime? get createdAt;/// When the order was archived, or null while it is still in the shop.
///
...
@JsonKey(name: 'deleted_at') DateTime? get deletedAt;/// What deleting this order — or restoring it — would do to the warehouse, in the server's
...
@JsonKey(name: 'stock_effect') StockEffect? get stockEffect;/// Who took the order. Null on the list — the server sends it with the full order only —
...
@JsonKey(name: 'created_by') OrderActor? get createdBy;
...
bool operator ==(Object other) {
  return identical(this, other) || (other.runtimeType == runtimeType&&other is Order&&(identical(other.id, id) || other.id == id)&&(identical(other.code, code) || other.code == code)&&(identical(other.status, status) || other.status == status)&&(identical(other.statusLabel, statusLabel) || other.statusLabel == statusLabel)&&(identical(other.productionFlowLabel, productionFlowLabel) || other.productionFlowLabel == productionFlowLabel)&&(identical(other.isFinal, isFinal) || other.isFinal == isFinal)&&(identical(other.isClosed, isClosed) || other.isClosed == isClosed)&&(identical(other.isUrgent, isUrgent) || other.isUrgent == isUrgent)&&(identical(other.readyMessageApplies, readyMessageApplies) || other.readyMessageApplies == readyMessageApplies)&&(identical(other.isReadyMessageSent, isReadyMessageSent) || other.isReadyMessageSent == isReadyMessageSent)&&(identical(other.readyMessageSentAt, readyMessageSentAt) || other.readyMessageSentAt == readyMessageSentAt)&&(identical(other.readyMessageSentBy, readyMessageSentBy) || other.readyMessageSentBy == readyMessageSentBy)&&(identical(other.depositExpectedAmount, depositExpectedAmount) || other.depositExpectedAmount == depositExpectedAmount)&&(identical(other.depositExpectedMethod, depositExpectedMethod) || other.depositExpectedMethod == depositExpectedMethod)&&(identical(other.depositExpectedMethodLabel, depositExpectedMethodLabel) || other.depositExpectedMethodLabel == depositExpectedMethodLabel)&&(identical(other.depositPaidAt, depositPaidAt) || other.depositPaidAt == depositPaidAt)&&(identical(other.isDepositReceived, isDepositReceived) || other.isDepositReceived == isDepositReceived)&&(identical(other.depositConfirmedAt, depositConfirmedAt) || other.depositConfirmedAt == depositConfirmedAt)&&(identical(other.canConfirmDeposit, canConfirmDeposit) || other.canConfirmDeposit == canConfirmDeposit)&&(identical(other.awaitsDepositConfirmation, awaitsDepositConfirmation) || other.awaitsDepositConfirmation == awaitsDepositConfirmation)&&(identical(other.depositClaimedBy, depositClaimedBy) || other.depositClaimedBy == depositClaimedBy)&&(identical(other.depositConfirmedBy, depositConfirmedBy) || other.depositConfirmedBy == depositConfirmedBy)&&(identical(other.isPartiallyDelivered, isPartiallyDelivered) || other.isPartiallyDelivered == isPartiallyDelivered)&&const DeepCollectionEquality().equals(other.availableTransitions, availableTransitions)&&(identical(other.customerId, customerId) || other.customerId == customerId)&&(identical(other.cityId, cityId) || other.cityId == cityId)&&(identical(other.designSource, designSource) || other.designSource == designSource)&&(identical(other.cityName, cityName) || other.cityName == cityName)&&(identical(other.fulfilmentTypeLabel, fulfilmentTypeLabel) || other.fulfilmentTypeLabel == fulfilmentTypeLabel)&&(identical(other.isOfficePickup, isOfficePickup) || other.isOfficePickup == isOfficePickup)&&(identical(other.designSourceLabel, designSourceLabel) || other.designSourceLabel == designSourceLabel)&&(identical(other.itemsTotal, itemsTotal) || other.itemsTotal == itemsTotal)&&(identical(other.designFee, designFee) || other.designFee == designFee)&&(identical(other.deliveryPrice, deliveryPrice) || other.deliveryPrice == deliveryPrice)&&(identical(other.discount, discount) || other.discount == discount)&&(identical(other.additionalCost, additionalCost) || other.additionalCost == additionalCost)&&(identical(other.additionalCostReason, additionalCostReason) || other.additionalCostReason == additionalCostReason)&&(identical(other.additionalCostReasonLabel, additionalCostReasonLabel) || other.additionalCostReasonLabel == additionalCostReasonLabel)&&(identical(other.additionalCostNote, additionalCostNote) || other.additionalCostNote == additionalCostNote)&&(identical(other.grandTotal, grandTotal) || other.grandTotal == grandTotal)&&(identical(other.paidAmount, paidAmount) || other.paidAmount == paidAmount)&&(identical(other.writtenOffAmount, writtenOffAmount) || other.writtenOffAmount == writtenOffAmount)&&(identical(other.carrierSettledAmount, carrierSettledAmount) || other.carrierSettledAmount == carrierSettledAmount)&&(identical(other.remainingAmount, remainingAmount) || other.remainingAmount == remainingAmount)&&(identical(other.paymentStatus, paymentStatus) || other.paymentStatus == paymentStatus)&&(identical(other.paymentStatusLabel, paymentStatusLabel) || other.paymentStatusLabel == paymentStatusLabel)&&(identical(other.hasUnrecordedMoney, hasUnrecordedMoney) || other.hasUnrecordedMoney == hasUnrecordedMoney)&&(identical(other.collectedAmount, collectedAmount) || other.collectedAmount == collectedAmount)&&(identical(other.customer, customer) || other.customer == customer)&&(identical(other.regionId, regionId) || other.regionId == regionId)&&(identical(other.customerShopId, customerShopId) || other.customerShopId == customerShopId)&&(identical(other.regionName, regionName) || other.regionName == regionName)&&(identical(other.customerShopName, customerShopName) || other.customerShopName == customerShopName)&&(identical(other.recipientName, recipientName) || other.recipientName == recipientName)&&(identical(other.recipientPhone, recipientPhone) || other.recipientPhone == recipientPhone)&&(identical(other.addressDetails, addressDetails) || other.addressDetails == addressDetails)&&(identical(other.notes, notes) || other.notes == notes)&&(identical(other.shippingCompany, shippingCompany) || other.shippingCompany == shippingCompany)&&(identical(other.trackingNumber, trackingNumber) || other.trackingNumber == trackingNumber)&&(identical(other.nawrisParcel, nawrisParcel) || other.nawrisParcel == nawrisParcel)&&(identical(other.courierPhone, courierPhone) || other.courierPhone == courierPhone)&&(identical(other.cancellationReason, cancellationReason) || other.cancellationReason == cancellationReason)&&(identical(other.reinstateTo, reinstateTo) || other.reinstateTo == reinstateTo)&&(identical(other.reinstateToLabel, reinstateToLabel) || other.reinstateToLabel == reinstateToLabel)&&(identical(other.canUnsettle, canUnsettle) || other.canUnsettle == canUnsettle)&&(identical(other.undoDeliveryTo, undoDeliveryTo) || other.undoDeliveryTo == undoDeliveryTo)&&(identical(other.undoDeliveryToLabel, undoDeliveryToLabel) || other.undoDeliveryToLabel == undoDeliveryToLabel)&&(identical(other.progress, progress) || other.progress == progress)&&(identical(other.itemsAreEditable, itemsAreEditable) || other.itemsAreEditable == itemsAreEditable)&&(identical(other.designsAreEditable, designsAreEditable) || other.designsAreEditable == designsAreEditable)&&(identical(other.destinationIsEditable, destinationIsEditable) || other.destinationIsEditable == destinationIsEditable)&&(identical(other.itemsCount, itemsCount) || other.itemsCount == itemsCount)&&const DeepCollectionEquality().equals(other.items, items)&&const DeepCollectionEquality().equals(other.designs, designs)&&const DeepCollectionEquality().equals(other.transitions, transitions)&&(identical(other.totalCogs, totalCogs) || other.totalCogs == totalCogs)&&(identical(other.grossProfit, grossProfit) || other.grossProfit == grossProfit)&&(identical(other.fulfillmentWarehouseId, fulfillmentWarehouseId) || other.fulfillmentWarehouseId == fulfillmentWarehouseId)&&(identical(other.stockDeductedAt, stockDeductedAt) || other.stockDeductedAt == stockDeductedAt)&&(identical(other.vendorId, vendorId) || other.vendorId == vendorId)&&(identical(other.vendorName, vendorName) || other.vendorName == vendorName)&&(identical(other.manufacturingStartedAt, manufacturingStartedAt) || other.manufacturingStartedAt == manufacturingStartedAt)&&(identical(other.totalWeight, totalWeight) || other.totalWeight == totalWeight)&&(identical(other.placedAt, placedAt) || other.placedAt == placedAt)&&(identical(other.deliveredAt, deliveredAt) || other.deliveredAt == deliveredAt)&&(identical(other.settledAt, settledAt) || other.settledAt == settledAt)&&(identical(other.createdAt, createdAt) || other.createdAt == createdAt)&&(identical(other.deletedAt, deletedAt) || other.deletedAt == deletedAt)&&(identical(other.stockEffect, stockEffect) || other.stockEffect == stockEffect)&&(identical(other.createdBy, createdBy) || other.createdBy == createdBy));
}

@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,code,status,statusLabel,productionFlowLabel,isFinal,isClosed,isUrgent,readyMessageApplies,isReadyMessageSent,readyMessageSentAt,readyMessageSentBy,depositExpectedAmount,depositExpectedMethod,depositExpectedMethodLabel,depositPaidAt,isDepositReceived,depositConfirmedAt,canConfirmDeposit,awaitsDepositConfirmation,depositClaimedBy,depositConfirmedBy,isPartiallyDelivered,const DeepCollectionEquality().hash(availableTransitions),customerId,cityId,designSource,cityName,fulfilmentTypeLabel,isOfficePickup,designSourceLabel,itemsTotal,designFee,deliveryPrice,discount,additionalCost,additionalCostReason,additionalCostReasonLabel,additionalCostNote,grandTotal,paidAmount,writtenOffAmount,carrierSettledAmount,remainingAmount,paymentStatus,paymentStatusLabel,hasUnrecordedMoney,collectedAmount,customer,regionId,customerShopId,regionName,customerShopName,recipientName,recipientPhone,addressDetails,notes,shippingCompany,trackingNumber,nawrisParcel,courierPhone,cancellationReason,reinstateTo,reinstateToLabel,canUnsettle,undoDeliveryTo,undoDeliveryToLabel,progress,itemsAreEditable,designsAreEditable,destinationIsEditable,itemsCount,const DeepCollectionEquality().hash(items),const DeepCollectionEquality().hash(designs),const DeepCollectionEquality().hash(transitions),totalCogs,grossProfit,fulfillmentWarehouseId,stockDeductedAt,vendorId,vendorName,manufacturingStartedAt,totalWeight,placedAt,deliveredAt,settledAt,createdAt,deletedAt,stockEffect,createdBy]);

@override
String toString() {
  return 'Order(id: $id, code: $code, status: $status, statusLabel: $statusLabel, productionFlowLabel: $productionFlowLabel, isFinal: $isFinal, isClosed: $isClosed, isUrgent: $isUrgent, readyMessageApplies: $readyMessageApplies, isReadyMessageSent: $isReadyMessageSent, readyMessageSentAt: $readyMessageSentAt, readyMessageSentBy: $readyMessageSentBy, depositExpectedAmount: $depositExpectedAmount, depositExpectedMethod: $depositExpectedMethod, depositExpectedMethodLabel: $depositExpectedMethodLabel, depositPaidAt: $depositPaidAt, isDepositReceived: $isDepositReceived, depositConfirmedAt: $depositConfirmedAt, canConfirmDeposit: $canConfirmDeposit, awaitsDepositConfirmation: $awaitsDepositConfirmation, depositClaimedBy: $depositClaimedBy, depositConfirmedBy: $depositConfirmedBy, isPartiallyDelivered: $isPartiallyDelivered, availableTransitions: $availableTransitions, customerId: $customerId, cityId: $cityId, designSource: $designSource, cityName: $cityName, fulfilmentTypeLabel: $fulfilmentTypeLabel, isOfficePickup: $isOfficePickup, designSourceLabel: $designSourceLabel, itemsTotal: $itemsTotal, designFee: $designFee, deliveryPrice: $deliveryPrice, discount: $discount, additionalCost: $additionalCost, additionalCostReason: $additionalCostReason, additionalCostReasonLabel: $additionalCostReasonLabel, additionalCostNote: $additionalCostNote, grandTotal: $grandTotal, paidAmount: $paidAmount, writtenOffAmount: $writtenOffAmount, carrierSettledAmount: $carrierSettledAmount, remainingAmount: $remainingAmount, paymentStatus: $paymentStatus, paymentStatusLabel: $paymentStatusLabel, hasUnrecordedMoney: $hasUnrecordedMoney, collectedAmount: $collectedAmount, customer: $customer, regionId: $regionId, customerShopId: $customerShopId, regionName: $regionName, customerShopName: $customerShopName, recipientName: $recipientName, recipientPhone: $recipientPhone, addressDetails: $addressDetails, notes: $notes, shippingCompany: $shippingCompany, trackingNumber: $trackingNumber, nawrisParcel: $nawrisParcel, courierPhone: $courierPhone, cancellationReason: $cancellationReason, reinstateTo: $reinstateTo, reinstateToLabel: $reinstateToLabel, canUnsettle: $canUnsettle, undoDeliveryTo: $undoDeliveryTo, undoDeliveryToLabel: $undoDeliveryToLabel, progress: $progress, itemsAreEditable: $itemsAreEditable, designsAreEditable: $designsAreEditable, destinationIsEditable: $destinationIsEditable, itemsCount: $itemsCount, items: $items, designs: $designs, transitions: $transitions, totalCogs: $totalCogs, grossProfit: $grossProfit, fulfillmentWarehouseId: $fulfillmentWarehouseId, stockDeductedAt: $stockDeductedAt, vendorId: $vendorId, vendorName: $vendorName, manufacturingStartedAt: $manufacturingStartedAt, totalWeight: $totalWeight, placedAt: $placedAt, deliveredAt: $deliveredAt, settledAt: $settledAt, createdAt: $createdAt, deletedAt: $deletedAt, stockEffect: $stockEffect, createdBy: $createdBy)';
}
...
@useResult
$Res call({
 int id, String code,@JsonKey(unknownEnumValue: OrderStatus.unknown) OrderStatus status,@JsonKey(name: 'status_label') String statusLabel,@JsonKey(name: 'production_flow_label') String productionFlowLabel,@JsonKey(name: 'is_final') bool isFinal,@JsonKey(name: 'is_closed') bool isClosed,@JsonKey(name: 'is_urgent') bool isUrgent,@JsonKey(name: 'ready_message_applies') bool readyMessageApplies,@JsonKey(name: 'is_ready_message_sent') bool isReadyMessageSent,@JsonKey(name: 'ready_message_sent_at') DateTime? readyMessageSentAt,@JsonKey(name: 'ready_message_sent_by') OrderActor? readyMessageSentBy,@JsonKey(name: 'deposit_expected_amount') String? depositExpectedAmount,@JsonKey(name: 'deposit_expected_method') String? depositExpectedMethod,@JsonKey(name: 'deposit_expected_method_label') String? depositExpectedMethodLabel,@JsonKey(name: 'deposit_paid_at') DateTime? depositPaidAt,@JsonKey(name: 'is_deposit_received') bool isDepositReceived,@JsonKey(name: 'deposit_confirmed_at') DateTime? depositConfirmedAt,@JsonKey(name: 'can_confirm_deposit') bool canConfirmDeposit,@JsonKey(name: 'awaits_deposit_confirmation') bool awaitsDepositConfirmation,@JsonKey(name: 'deposit_claimed_by') OrderActor? depositClaimedBy,@JsonKey(name: 'deposit_confirmed_by') OrderActor? depositConfirmedBy,@JsonKey(name: 'is_partially_delivered') bool? isPartiallyDelivered,@JsonKey(name: 'available_transitions') List<OrderTransition> availableTransitions,@JsonKey(name: 'customer_id') int customerId,@JsonKey(name: 'city_id') int cityId,@JsonKey(name: 'design_source') String designSource,@JsonKey(name: 'city_name') String cityName,@JsonKey(name: 'fulfilment_type_label') String fulfilmentTypeLabel,@JsonKey(name: 'is_office_pickup') bool isOfficePickup,@JsonKey(name: 'design_source_label') String designSourceLabel,@JsonKey(name: 'items_total') String itemsTotal,@JsonKey(name: 'design_fee') String designFee,@JsonKey(name: 'delivery_price') String deliveryPrice, String discount,@JsonKey(name: 'additional_cost') String additionalCost,@JsonKey(name: 'additional_cost_reason', unknownEnumValue: AdditionalCostReason.unknown) AdditionalCostReason? additionalCostReason,@JsonKey(name: 'additional_cost_reason_label') String? additionalCostReasonLabel,@JsonKey(name: 'additional_cost_note') String? additionalCostNote,@JsonKey(name: 'grand_total') String grandTotal,@JsonKey(name: 'paid_amount') String paidAmount,@JsonKey(name: 'written_off_amount') String writtenOffAmount,@JsonKey(name: 'carrier_settled_amount') String carrierSettledAmount,@JsonKey(name: 'remaining_amount') String remainingAmount,@JsonKey(name: 'payment_status', unknownEnumValue: PaymentStatus.unknown) PaymentStatus paymentStatus,@JsonKey(name: 'payment_status_label') String paymentStatusLabel,@JsonKey(name: 'has_unrecorded_money') bool hasUnrecordedMoney,@JsonKey(name: 'collected_amount') String? collectedAmount, Customer? customer,@JsonKey(name: 'region_id') int? regionId,@JsonKey(name: 'customer_shop_id') int? customerShopId,@JsonKey(name: 'region_name') String? regionName,@JsonKey(name: 'customer_shop_name') String? customerShopName,@JsonKey(name: 'recipient_name') String? recipientName,@JsonKey(name: 'recipient_phone') String? recipientPhone,@JsonKey(name: 'address_details') String? addressDetails, String? notes,@JsonKey(name: 'shipping_company') String? shippingCompany,@JsonKey(name: 'tracking_number') String? trackingNumber,@JsonKey(name: 'nawris_parcel') NawrisParcelRef? nawrisParcel,@JsonKey(name: 'courier_phone') String? courierPhone,@JsonKey(name: 'cancellation_reason') String? cancellationReason,@JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown) OrderStatus? reinstateTo,@JsonKey(name: 'reinstate_to_label') String? reinstateToLabel,@JsonKey(name: 'can_unsettle') bool canUnsettle,@JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown) OrderStatus? undoDeliveryTo,@JsonKey(name: 'undo_delivery_to_label') String? undoDeliveryToLabel, OrderProgress progress,@JsonKey(name: 'items_are_editable') bool itemsAreEditable,@JsonKey(name: 'designs_are_editable') bool designsAreEditable,@JsonKey(name: 'destination_is_editable') bool destinationIsEditable,@JsonKey(name: 'items_count') int? itemsCount, List<OrderItem>? items, List<OrderDesign>? designs, List<OrderTransitionRecord>? transitions,@JsonKey(name: 'total_cogs') String? totalCogs,@JsonKey(name: 'gross_profit') String? grossProfit,@JsonKey(name: 'fulfillment_warehouse_id') int? fulfillmentWarehouseId,@JsonKey(name: 'stock_deducted_at') DateTime? stockDeductedAt,@JsonKey(name: 'vendor_id') int? vendorId,@JsonKey(name: 'vendor_name') String? vendorName,@JsonKey(name: 'manufacturing_started_at') DateTime? manufacturingStartedAt,@JsonKey(name: 'total_weight') String? totalWeight,@JsonKey(name: 'placed_at') DateTime? placedAt,@JsonKey(name: 'delivered_at') DateTime? deliveredAt,@JsonKey(name: 'settled_at') DateTime? settledAt,@JsonKey(name: 'created_at') DateTime? createdAt,@JsonKey(name: 'deleted_at') DateTime? deletedAt,@JsonKey(name: 'stock_effect') StockEffect? stockEffect,@JsonKey(name: 'created_by') OrderActor? createdBy
});


$OrderActorCopyWith<$Res>? get readyMessageSentBy;$OrderActorCopyWith<$Res>? get depositClaimedBy;$OrderActorCopyWith<$Res>? get depositConfirmedBy;$CustomerCopyWith<$Res>? get customer;$NawrisParcelRefCopyWith<$Res>? get nawrisParcel;$OrderProgressCopyWith<$Res> get progress;$StockEffectCopyWith<$Res>? get stockEffect;$OrderActorCopyWith<$Res>? get createdBy;
...
/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@pragma('vm:prefer-inline') @override $Res call({Object? id = null,Object? code = null,Object? status = null,Object? statusLabel = null,Object? productionFlowLabel = null,Object? isFinal = null,Object? isClosed = null,Object? isUrgent = null,Object? readyMessageApplies = null,Object? isReadyMessageSent = null,Object? readyMessageSentAt = freezed,Object? readyMessageSentBy = freezed,Object? depositExpectedAmount = freezed,Object? depositExpectedMethod = freezed,Object? depositExpectedMethodLabel = freezed,Object? depositPaidAt = freezed,Object? isDepositReceived = null,Object? depositConfirmedAt = freezed,Object? canConfirmDeposit = null,Object? awaitsDepositConfirmation = null,Object? depositClaimedBy = freezed,Object? depositConfirmedBy = freezed,Object? isPartiallyDelivered = freezed,Object? availableTransitions = null,Object? customerId = null,Object? cityId = null,Object? designSource = null,Object? cityName = null,Object? fulfilmentTypeLabel = null,Object? isOfficePickup = null,Object? designSourceLabel = null,Object? itemsTotal = null,Object? designFee = null,Object? deliveryPrice = null,Object? discount = null,Object? additionalCost = null,Object? additionalCostReason = freezed,Object? additionalCostReasonLabel = freezed,Object? additionalCostNote = freezed,Object? grandTotal = null,Object? paidAmount = null,Object? writtenOffAmount = null,Object? carrierSettledAmount = null,Object? remainingAmount = null,Object? paymentStatus = null,Object? paymentStatusLabel = null,Object? hasUnrecordedMoney = null,Object? collectedAmount = freezed,Object? customer = freezed,Object? regionId = freezed,Object? customerShopId = freezed,Object? regionName = freezed,Object? customerShopName = freezed,Object? recipientName = freezed,Object? recipientPhone = freezed,Object? addressDetails = freezed,Object? notes = freezed,Object? shippingCompany = freezed,Object? trackingNumber = freezed,Object? nawrisParcel = freezed,Object? courierPhone = freezed,Object? cancellationReason = freezed,Object? reinstateTo = freezed,Object? reinstateToLabel = freezed,Object? canUnsettle = null,Object? undoDeliveryTo = freezed,Object? undoDeliveryToLabel = freezed,Object? progress = null,Object? itemsAreEditable = null,Object? designsAreEditable = null,Object? destinationIsEditable = null,Object? itemsCount = freezed,Object? items = freezed,Object? designs = freezed,Object? transitions = freezed,Object? totalCogs = freezed,Object? grossProfit = freezed,Object? fulfillmentWarehouseId = freezed,Object? stockDeductedAt = freezed,Object? vendorId = freezed,Object? vendorName = freezed,Object? manufacturingStartedAt = freezed,Object? totalWeight = freezed,Object? placedAt = freezed,Object? deliveredAt = freezed,Object? settledAt = freezed,Object? createdAt = freezed,Object? deletedAt = freezed,Object? stockEffect = freezed,Object? createdBy = freezed,}) {
...
as bool,readyMessageApplies: null == readyMessageApplies ? _self.readyMessageApplies : readyMessageApplies // ignore: cast_nullable_to_non_nullable
as bool,isReadyMessageSent: null == isReadyMessageSent ? _self.isReadyMessageSent : isReadyMessageSent // ignore: cast_nullable_to_non_nullable
as bool,readyMessageSentAt: freezed == readyMessageSentAt ? _self.readyMessageSentAt : readyMessageSentAt // ignore: cast_nullable_to_non_nullable
as DateTime?,readyMessageSentBy: freezed == readyMessageSentBy ? _self.readyMessageSentBy : readyMessageSentBy // ignore: cast_nullable_to_non_nullable
as OrderActor?,depositExpectedAmount: freezed == depositExpectedAmount ? _self.depositExpectedAmount : depositExpectedAmount // ignore: cast_nullable_to_non_nullable
as String?,depositExpectedMethod: freezed == depositExpectedMethod ? _self.depositExpectedMethod : depositExpectedMethod // ignore: cast_nullable_to_non_nullable
as String?,depositExpectedMethodLabel: freezed == depositExpectedMethodLabel ? _self.depositExpectedMethodLabel : depositExpectedMethodLabel // ignore: cast_nullable_to_non_nullable
as String?,depositPaidAt: freezed == depositPaidAt ? _self.depositPaidAt : depositPaidAt // ignore: cast_nullable_to_non_nullable
as DateTime?,isDepositReceived: null == isDepositReceived ? _self.isDepositReceived : isDepositReceived // ignore: cast_nullable_to_non_nullable
as bool,depositConfirmedAt: freezed == depositConfirmedAt ? _self.depositConfirmedAt : depositConfirmedAt // ignore: cast_nullable_to_non_nullable
as DateTime?,canConfirmDeposit: null == canConfirmDeposit ? _self.canConfirmDeposit : canConfirmDeposit // ignore: cast_nullable_to_non_nullable
as bool,awaitsDepositConfirmation: null == awaitsDepositConfirmation ? _self.awaitsDepositConfirmation : awaitsDepositConfirmation // ignore: cast_nullable_to_non_nullable
as bool,depositClaimedBy: freezed == depositClaimedBy ? _self.depositClaimedBy : depositClaimedBy // ignore: cast_nullable_to_non_nullable
as OrderActor?,depositConfirmedBy: freezed == depositConfirmedBy ? _self.depositConfirmedBy : depositConfirmedBy // ignore: cast_nullable_to_non_nullable
as OrderActor?,isPartiallyDelivered: freezed == isPartiallyDelivered ? _self.isPartiallyDelivered : isPartiallyDelivered // ignore: cast_nullable_to_non_nullable
...
as String,carrierSettledAmount: null == carrierSettledAmount ? _self.carrierSettledAmount : carrierSettledAmount // ignore: cast_nullable_to_non_nullable
...
as String?,nawrisParcel: freezed == nawrisParcel ? _self.nawrisParcel : nawrisParcel // ignore: cast_nullable_to_non_nullable
as NawrisParcelRef?,courierPhone: freezed == courierPhone ? _self.courierPhone : courierPhone // ignore: cast_nullable_to_non_nullable
...
as String?,reinstateTo: freezed == reinstateTo ? _self.reinstateTo : reinstateTo // ignore: cast_nullable_to_non_nullable
as OrderStatus?,reinstateToLabel: freezed == reinstateToLabel ? _self.reinstateToLabel : reinstateToLabel // ignore: cast_nullable_to_non_nullable
as String?,canUnsettle: null == canUnsettle ? _self.canUnsettle : canUnsettle // ignore: cast_nullable_to_non_nullable
as bool,undoDeliveryTo: freezed == undoDeliveryTo ? _self.undoDeliveryTo : undoDeliveryTo // ignore: cast_nullable_to_non_nullable
as OrderStatus?,undoDeliveryToLabel: freezed == undoDeliveryToLabel ? _self.undoDeliveryToLabel : undoDeliveryToLabel // ignore: cast_nullable_to_non_nullable
...
as DateTime?,vendorId: freezed == vendorId ? _self.vendorId : vendorId // ignore: cast_nullable_to_non_nullable
as int?,vendorName: freezed == vendorName ? _self.vendorName : vendorName // ignore: cast_nullable_to_non_nullable
as String?,manufacturingStartedAt: freezed == manufacturingStartedAt ? _self.manufacturingStartedAt : manufacturingStartedAt // ignore: cast_nullable_to_non_nullable
as DateTime?,totalWeight: freezed == totalWeight ? _self.totalWeight : totalWeight // ignore: cast_nullable_to_non_nullable
...
as DateTime?,stockEffect: freezed == stockEffect ? _self.stockEffect : stockEffect // ignore: cast_nullable_to_non_nullable
as StockEffect?,createdBy: freezed == createdBy ? _self.createdBy : createdBy // ignore: cast_nullable_to_non_nullable
as OrderActor?,
  ));
}
/// Create a copy of Order
...
@override
@pragma('vm:prefer-inline')
$OrderActorCopyWith<$Res>? get readyMessageSentBy {
    if (_self.readyMessageSentBy == null) {
    return null;
  }

  return $OrderActorCopyWith<$Res>(_self.readyMessageSentBy!, (value) {
    return _then(_self.copyWith(readyMessageSentBy: value));
  });
}/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override
@pragma('vm:prefer-inline')
$OrderActorCopyWith<$Res>? get depositClaimedBy {
    if (_self.depositClaimedBy == null) {
    return null;
  }

  return $OrderActorCopyWith<$Res>(_self.depositClaimedBy!, (value) {
    return _then(_self.copyWith(depositClaimedBy: value));
  });
}/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override
@pragma('vm:prefer-inline')
$OrderActorCopyWith<$Res>? get depositConfirmedBy {
    if (_self.depositConfirmedBy == null) {
    return null;
  }

  return $OrderActorCopyWith<$Res>(_self.depositConfirmedBy!, (value) {
    return _then(_self.copyWith(depositConfirmedBy: value));
  });
}/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override
...
@override
@pragma('vm:prefer-inline')
$NawrisParcelRefCopyWith<$Res>? get nawrisParcel {
    if (_self.nawrisParcel == null) {
    return null;
  }

  return $NawrisParcelRefCopyWith<$Res>(_self.nawrisParcel!, (value) {
    return _then(_self.copyWith(nawrisParcel: value));
  });
}/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override
...
@override
@pragma('vm:prefer-inline')
$StockEffectCopyWith<$Res>? get stockEffect {
    if (_self.stockEffect == null) {
    return null;
  }

  return $StockEffectCopyWith<$Res>(_self.stockEffect!, (value) {
    return _then(_self.copyWith(stockEffect: value));
  });
}/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override
@pragma('vm:prefer-inline')
$OrderActorCopyWith<$Res>? get createdBy {
    if (_self.createdBy == null) {
    return null;
  }

  return $OrderActorCopyWith<$Res>(_self.createdBy!, (value) {
...
@optionalTypeArgs TResult maybeWhen<TResult extends Object?>(TResult Function( int id,  String code, @JsonKey(unknownEnumValue: OrderStatus.unknown)  OrderStatus status, @JsonKey(name: 'status_label')  String statusLabel, @JsonKey(name: 'production_flow_label')  String productionFlowLabel, @JsonKey(name: 'is_final')  bool isFinal, @JsonKey(name: 'is_closed')  bool isClosed, @JsonKey(name: 'is_urgent')  bool isUrgent, @JsonKey(name: 'ready_message_applies')  bool readyMessageApplies, @JsonKey(name: 'is_ready_message_sent')  bool isReadyMessageSent, @JsonKey(name: 'ready_message_sent_at')  DateTime? readyMessageSentAt, @JsonKey(name: 'ready_message_sent_by')  OrderActor? readyMessageSentBy, @JsonKey(name: 'deposit_expected_amount')  String? depositExpectedAmount, @JsonKey(name: 'deposit_expected_method')  String? depositExpectedMethod, @JsonKey(name: 'deposit_expected_method_label')  String? depositExpectedMethodLabel, @JsonKey(name: 'deposit_paid_at')  DateTime? depositPaidAt, @JsonKey(name: 'is_deposit_received')  bool isDepositReceived, @JsonKey(name: 'deposit_confirmed_at')  DateTime? depositConfirmedAt, @JsonKey(name: 'can_confirm_deposit')  bool canConfirmDeposit, @JsonKey(name: 'awaits_deposit_confirmation')  bool awaitsDepositConfirmation, @JsonKey(name: 'deposit_claimed_by')  OrderActor? depositClaimedBy, @JsonKey(name: 'deposit_confirmed_by')  OrderActor? depositConfirmedBy, @JsonKey(name: 'is_partially_delivered')  bool? isPartiallyDelivered, @JsonKey(name: 'available_transitions')  List<OrderTransition> availableTransitions, @JsonKey(name: 'customer_id')  int customerId, @JsonKey(name: 'city_id')  int cityId, @JsonKey(name: 'design_source')  String designSource, @JsonKey(name: 'city_name')  String cityName, @JsonKey(name: 'fulfilment_type_label')  String fulfilmentTypeLabel, @JsonKey(name: 'is_office_pickup')  bool isOfficePickup, @JsonKey(name: 'design_source_label')  String designSourceLabel, @JsonKey(name: 'items_total')  String itemsTotal, @JsonKey(name: 'design_fee')  String designFee, @JsonKey(name: 'delivery_price')  String deliveryPrice,  String discount, @JsonKey(name: 'additional_cost')  String additionalCost, @JsonKey(name: 'additional_cost_reason', unknownEnumValue: AdditionalCostReason.unknown)  AdditionalCostReason? additionalCostReason, @JsonKey(name: 'additional_cost_reason_label')  String? additionalCostReasonLabel, @JsonKey(name: 'additional_cost_note')  String? additionalCostNote, @JsonKey(name: 'grand_total')  String grandTotal, @JsonKey(name: 'paid_amount')  String paidAmount, @JsonKey(name: 'written_off_amount')  String writtenOffAmount, @JsonKey(name: 'carrier_settled_amount')  String carrierSettledAmount, @JsonKey(name: 'remaining_amount')  String remainingAmount, @JsonKey(name: 'payment_status', unknownEnumValue: PaymentStatus.unknown)  PaymentStatus paymentStatus, @JsonKey(name: 'payment_status_label')  String paymentStatusLabel, @JsonKey(name: 'has_unrecorded_money')  bool hasUnrecordedMoney, @JsonKey(name: 'collected_amount')  String? collectedAmount,  Customer? customer, @JsonKey(name: 'region_id')  int? regionId, @JsonKey(name: 'customer_shop_id')  int? customerShopId, @JsonKey(name: 'region_name')  String? regionName, @JsonKey(name: 'customer_shop_name')  String? customerShopName, @JsonKey(name: 'recipient_name')  String? recipientName, @JsonKey(name: 'recipient_phone')  String? recipientPhone, @JsonKey(name: 'address_details')  String? addressDetails,  String? notes, @JsonKey(name: 'shipping_company')  String? shippingCompany, @JsonKey(name: 'tracking_number')  String? trackingNumber, @JsonKey(name: 'nawris_parcel')  NawrisParcelRef? nawrisParcel, @JsonKey(name: 'courier_phone')  String? courierPhone, @JsonKey(name: 'cancellation_reason')  String? cancellationReason, @JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown)  OrderStatus? reinstateTo, @JsonKey(name: 'reinstate_to_label')  String? reinstateToLabel, @JsonKey(name: 'can_unsettle')  bool canUnsettle, @JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown)  OrderStatus? undoDeliveryTo, @JsonKey(name: 'undo_delivery_to_label')  String? undoDeliveryToLabel,  OrderProgress progress, @JsonKey(name: 'items_are_editable')  bool itemsAreEditable, @JsonKey(name: 'designs_are_editable')  bool designsAreEditable, @JsonKey(name: 'destination_is_editable')  bool destinationIsEditable, @JsonKey(name: 'items_count')  int? itemsCount,  List<OrderItem>? items,  List<OrderDesign>? designs,  List<OrderTransitionRecord>? transitions, @JsonKey(name: 'total_cogs')  String? totalCogs, @JsonKey(name: 'gross_profit')  String? grossProfit, @JsonKey(name: 'fulfillment_warehouse_id')  int? fulfillmentWarehouseId, @JsonKey(name: 'stock_deducted_at')  DateTime? stockDeductedAt, @JsonKey(name: 'vendor_id')  int? vendorId, @JsonKey(name: 'vendor_name')  String? vendorName, @JsonKey(name: 'manufacturing_started_at')  DateTime? manufacturingStartedAt, @JsonKey(name: 'total_weight')  String? totalWeight, @JsonKey(name: 'placed_at')  DateTime? placedAt, @JsonKey(name: 'delivered_at')  DateTime? deliveredAt, @JsonKey(name: 'settled_at')  DateTime? settledAt, @JsonKey(name: 'created_at')  DateTime? createdAt, @JsonKey(name: 'deleted_at')  DateTime? deletedAt, @JsonKey(name: 'stock_effect')  StockEffect? stockEffect, @JsonKey(name: 'created_by')  OrderActor? createdBy)?  $default,{required TResult orElse(),}) {final _that = this;
switch (_that) {
case _Order() when $default != null:
return $default(_that.id,_that.code,_that.status,_that.statusLabel,_that.productionFlowLabel,_that.isFinal,_that.isClosed,_that.isUrgent,_that.readyMessageApplies,_that.isReadyMessageSent,_that.readyMessageSentAt,_that.readyMessageSentBy,_that.depositExpectedAmount,_that.depositExpectedMethod,_that.depositExpectedMethodLabel,_that.depositPaidAt,_that.isDepositReceived,_that.depositConfirmedAt,_that.canConfirmDeposit,_that.awaitsDepositConfirmation,_that.depositClaimedBy,_that.depositConfirmedBy,_that.isPartiallyDelivered,_that.availableTransitions,_that.customerId,_that.cityId,_that.designSource,_that.cityName,_that.fulfilmentTypeLabel,_that.isOfficePickup,_that.designSourceLabel,_that.itemsTotal,_that.designFee,_that.deliveryPrice,_that.discount,_that.additionalCost,_that.additionalCostReason,_that.additionalCostReasonLabel,_that.additionalCostNote,_that.grandTotal,_that.paidAmount,_that.writtenOffAmount,_that.carrierSettledAmount,_that.remainingAmount,_that.paymentStatus,_that.paymentStatusLabel,_that.hasUnrecordedMoney,_that.collectedAmount,_that.customer,_that.regionId,_that.customerShopId,_that.regionName,_that.customerShopName,_that.recipientName,_that.recipientPhone,_that.addressDetails,_that.notes,_that.shippingCompany,_that.trackingNumber,_that.nawrisParcel,_that.courierPhone,_that.cancellationReason,_that.reinstateTo,_that.reinstateToLabel,_that.canUnsettle,_that.undoDeliveryTo,_that.undoDeliveryToLabel,_that.progress,_that.itemsAreEditable,_that.designsAreEditable,_that.destinationIsEditable,_that.itemsCount,_that.items,_that.designs,_that.transitions,_that.totalCogs,_that.grossProfit,_that.fulfillmentWarehouseId,_that.stockDeductedAt,_that.vendorId,_that.vendorName,_that.manufacturingStartedAt,_that.totalWeight,_that.placedAt,_that.deliveredAt,_that.settledAt,_that.createdAt,_that.deletedAt,_that.stockEffect,_that.createdBy);case _:
  return orElse();
...
@optionalTypeArgs TResult when<TResult extends Object?>(TResult Function( int id,  String code, @JsonKey(unknownEnumValue: OrderStatus.unknown)  OrderStatus status, @JsonKey(name: 'status_label')  String statusLabel, @JsonKey(name: 'production_flow_label')  String productionFlowLabel, @JsonKey(name: 'is_final')  bool isFinal, @JsonKey(name: 'is_closed')  bool isClosed, @JsonKey(name: 'is_urgent')  bool isUrgent, @JsonKey(name: 'ready_message_applies')  bool readyMessageApplies, @JsonKey(name: 'is_ready_message_sent')  bool isReadyMessageSent, @JsonKey(name: 'ready_message_sent_at')  DateTime? readyMessageSentAt, @JsonKey(name: 'ready_message_sent_by')  OrderActor? readyMessageSentBy, @JsonKey(name: 'deposit_expected_amount')  String? depositExpectedAmount, @JsonKey(name: 'deposit_expected_method')  String? depositExpectedMethod, @JsonKey(name: 'deposit_expected_method_label')  String? depositExpectedMethodLabel, @JsonKey(name: 'deposit_paid_at')  DateTime? depositPaidAt, @JsonKey(name: 'is_deposit_received')  bool isDepositReceived, @JsonKey(name: 'deposit_confirmed_at')  DateTime? depositConfirmedAt, @JsonKey(name: 'can_confirm_deposit')  bool canConfirmDeposit, @JsonKey(name: 'awaits_deposit_confirmation')  bool awaitsDepositConfirmation, @JsonKey(name: 'deposit_claimed_by')  OrderActor? depositClaimedBy, @JsonKey(name: 'deposit_confirmed_by')  OrderActor? depositConfirmedBy, @JsonKey(name: 'is_partially_delivered')  bool? isPartiallyDelivered, @JsonKey(name: 'available_transitions')  List<OrderTransition> availableTransitions, @JsonKey(name: 'customer_id')  int customerId, @JsonKey(name: 'city_id')  int cityId, @JsonKey(name: 'design_source')  String designSource, @JsonKey(name: 'city_name')  String cityName, @JsonKey(name: 'fulfilment_type_label')  String fulfilmentTypeLabel, @JsonKey(name: 'is_office_pickup')  bool isOfficePickup, @JsonKey(name: 'design_source_label')  String designSourceLabel, @JsonKey(name: 'items_total')  String itemsTotal, @JsonKey(name: 'design_fee')  String designFee, @JsonKey(name: 'delivery_price')  String deliveryPrice,  String discount, @JsonKey(name: 'additional_cost')  String additionalCost, @JsonKey(name: 'additional_cost_reason', unknownEnumValue: AdditionalCostReason.unknown)  AdditionalCostReason? additionalCostReason, @JsonKey(name: 'additional_cost_reason_label')  String? additionalCostReasonLabel, @JsonKey(name: 'additional_cost_note')  String? additionalCostNote, @JsonKey(name: 'grand_total')  String grandTotal, @JsonKey(name: 'paid_amount')  String paidAmount, @JsonKey(name: 'written_off_amount')  String writtenOffAmount, @JsonKey(name: 'carrier_settled_amount')  String carrierSettledAmount, @JsonKey(name: 'remaining_amount')  String remainingAmount, @JsonKey(name: 'payment_status', unknownEnumValue: PaymentStatus.unknown)  PaymentStatus paymentStatus, @JsonKey(name: 'payment_status_label')  String paymentStatusLabel, @JsonKey(name: 'has_unrecorded_money')  bool hasUnrecordedMoney, @JsonKey(name: 'collected_amount')  String? collectedAmount,  Customer? customer, @JsonKey(name: 'region_id')  int? regionId, @JsonKey(name: 'customer_shop_id')  int? customerShopId, @JsonKey(name: 'region_name')  String? regionName, @JsonKey(name: 'customer_shop_name')  String? customerShopName, @JsonKey(name: 'recipient_name')  String? recipientName, @JsonKey(name: 'recipient_phone')  String? recipientPhone, @JsonKey(name: 'address_details')  String? addressDetails,  String? notes, @JsonKey(name: 'shipping_company')  String? shippingCompany, @JsonKey(name: 'tracking_number')  String? trackingNumber, @JsonKey(name: 'nawris_parcel')  NawrisParcelRef? nawrisParcel, @JsonKey(name: 'courier_phone')  String? courierPhone, @JsonKey(name: 'cancellation_reason')  String? cancellationReason, @JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown)  OrderStatus? reinstateTo, @JsonKey(name: 'reinstate_to_label')  String? reinstateToLabel, @JsonKey(name: 'can_unsettle')  bool canUnsettle, @JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown)  OrderStatus? undoDeliveryTo, @JsonKey(name: 'undo_delivery_to_label')  String? undoDeliveryToLabel,  OrderProgress progress, @JsonKey(name: 'items_are_editable')  bool itemsAreEditable, @JsonKey(name: 'designs_are_editable')  bool designsAreEditable, @JsonKey(name: 'destination_is_editable')  bool destinationIsEditable, @JsonKey(name: 'items_count')  int? itemsCount,  List<OrderItem>? items,  List<OrderDesign>? designs,  List<OrderTransitionRecord>? transitions, @JsonKey(name: 'total_cogs')  String? totalCogs, @JsonKey(name: 'gross_profit')  String? grossProfit, @JsonKey(name: 'fulfillment_warehouse_id')  int? fulfillmentWarehouseId, @JsonKey(name: 'stock_deducted_at')  DateTime? stockDeductedAt, @JsonKey(name: 'vendor_id')  int? vendorId, @JsonKey(name: 'vendor_name')  String? vendorName, @JsonKey(name: 'manufacturing_started_at')  DateTime? manufacturingStartedAt, @JsonKey(name: 'total_weight')  String? totalWeight, @JsonKey(name: 'placed_at')  DateTime? placedAt, @JsonKey(name: 'delivered_at')  DateTime? deliveredAt, @JsonKey(name: 'settled_at')  DateTime? settledAt, @JsonKey(name: 'created_at')  DateTime? createdAt, @JsonKey(name: 'deleted_at')  DateTime? deletedAt, @JsonKey(name: 'stock_effect')  StockEffect? stockEffect, @JsonKey(name: 'created_by')  OrderActor? createdBy)  $default,) {final _that = this;
switch (_that) {
case _Order():
return $default(_that.id,_that.code,_that.status,_that.statusLabel,_that.productionFlowLabel,_that.isFinal,_that.isClosed,_that.isUrgent,_that.readyMessageApplies,_that.isReadyMessageSent,_that.readyMessageSentAt,_that.readyMessageSentBy,_that.depositExpectedAmount,_that.depositExpectedMethod,_that.depositExpectedMethodLabel,_that.depositPaidAt,_that.isDepositReceived,_that.depositConfirmedAt,_that.canConfirmDeposit,_that.awaitsDepositConfirmation,_that.depositClaimedBy,_that.depositConfirmedBy,_that.isPartiallyDelivered,_that.availableTransitions,_that.customerId,_that.cityId,_that.designSource,_that.cityName,_that.fulfilmentTypeLabel,_that.isOfficePickup,_that.designSourceLabel,_that.itemsTotal,_that.designFee,_that.deliveryPrice,_that.discount,_that.additionalCost,_that.additionalCostReason,_that.additionalCostReasonLabel,_that.additionalCostNote,_that.grandTotal,_that.paidAmount,_that.writtenOffAmount,_that.carrierSettledAmount,_that.remainingAmount,_that.paymentStatus,_that.paymentStatusLabel,_that.hasUnrecordedMoney,_that.collectedAmount,_that.customer,_that.regionId,_that.customerShopId,_that.regionName,_that.customerShopName,_that.recipientName,_that.recipientPhone,_that.addressDetails,_that.notes,_that.shippingCompany,_that.trackingNumber,_that.nawrisParcel,_that.courierPhone,_that.cancellationReason,_that.reinstateTo,_that.reinstateToLabel,_that.canUnsettle,_that.undoDeliveryTo,_that.undoDeliveryToLabel,_that.progress,_that.itemsAreEditable,_that.designsAreEditable,_that.destinationIsEditable,_that.itemsCount,_that.items,_that.designs,_that.transitions,_that.totalCogs,_that.grossProfit,_that.fulfillmentWarehouseId,_that.stockDeductedAt,_that.vendorId,_that.vendorName,_that.manufacturingStartedAt,_that.totalWeight,_that.placedAt,_that.deliveredAt,_that.settledAt,_that.createdAt,_that.deletedAt,_that.stockEffect,_that.createdBy);case _:
  throw StateError('Unexpected subclass');
...
class _Order extends Order {
  const _Order({required this.id, required this.code, @JsonKey(unknownEnumValue: OrderStatus.unknown) required this.status, @JsonKey(name: 'status_label') required this.statusLabel, @JsonKey(name: 'production_flow_label') this.productionFlowLabel = '', @JsonKey(name: 'is_final') required this.isFinal, @JsonKey(name: 'is_closed') this.isClosed = false, @JsonKey(name: 'is_urgent') this.isUrgent = false, @JsonKey(name: 'ready_message_applies') this.readyMessageApplies = false, @JsonKey(name: 'is_ready_message_sent') this.isReadyMessageSent = false, @JsonKey(name: 'ready_message_sent_at') this.readyMessageSentAt, @JsonKey(name: 'ready_message_sent_by') this.readyMessageSentBy, @JsonKey(name: 'deposit_expected_amount') this.depositExpectedAmount, @JsonKey(name: 'deposit_expected_method') this.depositExpectedMethod, @JsonKey(name: 'deposit_expected_method_label') this.depositExpectedMethodLabel, @JsonKey(name: 'deposit_paid_at') this.depositPaidAt, @JsonKey(name: 'is_deposit_received') this.isDepositReceived = false, @JsonKey(name: 'deposit_confirmed_at') this.depositConfirmedAt, @JsonKey(name: 'can_confirm_deposit') this.canConfirmDeposit = false, @JsonKey(name: 'awaits_deposit_confirmation') this.awaitsDepositConfirmation = false, @JsonKey(name: 'deposit_claimed_by') this.depositClaimedBy, @JsonKey(name: 'deposit_confirmed_by') this.depositConfirmedBy, @JsonKey(name: 'is_partially_delivered') this.isPartiallyDelivered, @JsonKey(name: 'available_transitions') final  List<OrderTransition> availableTransitions = const <OrderTransition>[], @JsonKey(name: 'customer_id') required this.customerId, @JsonKey(name: 'city_id') required this.cityId, @JsonKey(name: 'design_source') required this.designSource, @JsonKey(name: 'city_name') required this.cityName, @JsonKey(name: 'fulfilment_type_label') required this.fulfilmentTypeLabel, @JsonKey(name: 'is_office_pickup') required this.isOfficePickup, @JsonKey(name: 'design_source_label') required this.designSourceLabel, @JsonKey(name: 'items_total') required this.itemsTotal, @JsonKey(name: 'design_fee') required this.designFee, @JsonKey(name: 'delivery_price') required this.deliveryPrice, required this.discount, @JsonKey(name: 'additional_cost') this.additionalCost = '0.00', @JsonKey(name: 'additional_cost_reason', unknownEnumValue: AdditionalCostReason.unknown) this.additionalCostReason, @JsonKey(name: 'additional_cost_reason_label') this.additionalCostReasonLabel, @JsonKey(name: 'additional_cost_note') this.additionalCostNote, @JsonKey(name: 'grand_total') required this.grandTotal, @JsonKey(name: 'paid_amount') this.paidAmount = '0.00', @JsonKey(name: 'written_off_amount') this.writtenOffAmount = '0.00', @JsonKey(name: 'carrier_settled_amount') this.carrierSettledAmount = '0.00', @JsonKey(name: 'remaining_amount') this.remainingAmount = '0.00', @JsonKey(name: 'payment_status', unknownEnumValue: PaymentStatus.unknown) this.paymentStatus = PaymentStatus.unpaid, @JsonKey(name: 'payment_status_label') this.paymentStatusLabel = '', @JsonKey(name: 'has_unrecorded_money') this.hasUnrecordedMoney = false, @JsonKey(name: 'collected_amount') this.collectedAmount, this.customer, @JsonKey(name: 'region_id') this.regionId, @JsonKey(name: 'customer_shop_id') this.customerShopId, @JsonKey(name: 'region_name') this.regionName, @JsonKey(name: 'customer_shop_name') this.customerShopName, @JsonKey(name: 'recipient_name') this.recipientName, @JsonKey(name: 'recipient_phone') this.recipientPhone, @JsonKey(name: 'address_details') this.addressDetails, this.notes, @JsonKey(name: 'shipping_company') this.shippingCompany, @JsonKey(name: 'tracking_number') this.trackingNumber, @JsonKey(name: 'nawris_parcel') this.nawrisParcel, @JsonKey(name: 'courier_phone') this.courierPhone, @JsonKey(name: 'cancellation_reason') this.cancellationReason, @JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown) this.reinstateTo, @JsonKey(name: 'reinstate_to_label') this.reinstateToLabel, @JsonKey(name: 'can_unsettle') this.canUnsettle = false, @JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown) this.undoDeliveryTo, @JsonKey(name: 'undo_delivery_to_label') this.undoDeliveryToLabel, this.progress = OrderProgress.unknown, @JsonKey(name: 'items_are_editable') this.itemsAreEditable = false, @JsonKey(name: 'designs_are_editable') this.designsAreEditable = false, @JsonKey(name: 'destination_is_editable') this.destinationIsEditable = false, @JsonKey(name: 'items_count') this.itemsCount, final  List<OrderItem>? items, final  List<OrderDesign>? designs, final  List<OrderTransitionRecord>? transitions, @JsonKey(name: 'total_cogs') this.totalCogs, @JsonKey(name: 'gross_profit') this.grossProfit, @JsonKey(name: 'fulfillment_warehouse_id') this.fulfillmentWarehouseId, @JsonKey(name: 'stock_deducted_at') this.stockDeductedAt, @JsonKey(name: 'vendor_id') this.vendorId, @JsonKey(name: 'vendor_name') this.vendorName, @JsonKey(name: 'manufacturing_started_at') this.manufacturingStartedAt, @JsonKey(name: 'total_weight') this.totalWeight, @JsonKey(name: 'placed_at') this.placedAt, @JsonKey(name: 'delivered_at') this.deliveredAt, @JsonKey(name: 'settled_at') this.settledAt, @JsonKey(name: 'created_at') this.createdAt, @JsonKey(name: 'deleted_at') this.deletedAt, @JsonKey(name: 'stock_effect') this.stockEffect, @JsonKey(name: 'created_by') this.createdBy}): _availableTransitions = availableTransitions,_items = items,_designs = designs,_transitions = transitions,super._();
...
/// Defaulted false, so an order from a build of the API that predates this reads as «السؤال
/// لا ينطبق» and the section is simply not drawn.
@override@JsonKey(name: 'ready_message_applies') final  bool readyMessageApplies;
/// Whether somebody has said the customer was told.
///
/// **A record of work done outside this app** — the message goes out on WhatsApp or by
/// telephone — so nothing here observes it and nothing derives it. See [readyMessageSentBy].
@override@JsonKey(name: 'is_ready_message_sent') final  bool isReadyMessageSent;
@override@JsonKey(name: 'ready_message_sent_at') final  DateTime? readyMessageSentAt;
/// Who said so. **Null on a list row** — the server sends the name with the full order only,
/// because no card shows it and a page of twenty would be a query per row.
@override@JsonKey(name: 'ready_message_sent_by') final  OrderActor? readyMessageSentBy;
// ── العربون ──────────────────────────────────────────────────────────────────────────
//
// **The arrangement, the claim, and the confirmation — three different facts.** What was
// agreed is not money that has moved ([paidAmount] is where a real deposit shows up); the
...
/// What عربون was asked of the customer, and how it was to be paid. All three null on an
/// order nobody asked a deposit of, which is most of them.
@override@JsonKey(name: 'deposit_expected_amount') final  String? depositExpectedAmount;
@override@JsonKey(name: 'deposit_expected_method') final  String? depositExpectedMethod;
@override@JsonKey(name: 'deposit_expected_method_label') final  String? depositExpectedMethodLabel;
/// When the order was *said* to have been paid. Cleared if the claim is walked back.
@override@JsonKey(name: 'deposit_paid_at') final  DateTime? depositPaidAt;
/// An employee's tick that the money really landed — **a statement, not arithmetic.** It
/// gates nothing: an order at false prints, ships and settles exactly like one at true, and
/// no rule here may start reading it. See ORDER-DEPOSIT.md.
@override@JsonKey(name: 'is_deposit_received') final  bool isDepositReceived;
@override@JsonKey(name: 'deposit_confirmed_at') final  DateTime? depositConfirmedAt;
...
/// **Defaults to false, not true.** A build talking to an API that predates this greys the
/// switch, which is the safe way round.
@override@JsonKey(name: 'can_confirm_deposit') final  bool canConfirmDeposit;
/// A deposit declared paid that nobody has confirmed — the accountant's queue.
@override@JsonKey(name: 'awaits_deposit_confirmation') final  bool awaitsDepositConfirmation;
/// Who said it was paid, and who confirmed it. **Null on a list row** — the server sends
/// the names with the full order only, exactly as it does for [readyMessageSentBy].
@override@JsonKey(name: 'deposit_claimed_by') final  OrderActor? depositClaimedBy;
@override@JsonKey(name: 'deposit_confirmed_by') final  OrderActor? depositConfirmedBy;
...
/// false: it is absent from any payload that did not load the lines, and «لم يُسأل» is not
/// «لا».
@override@JsonKey(name: 'is_partially_delivered') final  bool? isPartiallyDelivered;
...
/// Defaulted like its two neighbours: an order from a server that predates the carrier
/// integration had nothing settled at one, and zero is what such a server means.
@override@JsonKey(name: 'carrier_settled_amount') final  String carrierSettledAmount;
...
/// on one whose parcel is still waiting for its code — the server omits the key rather than
/// sending an empty one, so null here means «no code», never «code unknown».
@override@JsonKey(name: 'nawris_parcel') final  NawrisParcelRef? nawrisParcel;
...
/// somebody taps rather than after. Sent only with a single order, never on a list — see
/// `OrderResource` for why the list does not pay for the timeline read.
@override@JsonKey(name: 'reinstate_to', unknownEnumValue: OrderStatus.unknown) final  OrderStatus? reinstateTo;
/// The Arabic the server chose for [reinstateTo]. Rendered as-is, for the reason
/// [statusLabel] gives: a status this build has never heard of still reads correctly.
@override@JsonKey(name: 'reinstate_to_label') final  String? reinstateToLabel;
...
/// delivered, no grant, a partial delivery, or a timeline that does not say. A delivery Nawris
/// reported still carries it; the server refuses that one with the reason.
@override@JsonKey(name: 'undo_delivery_to', unknownEnumValue: OrderStatus.unknown) final  OrderStatus? undoDeliveryTo;
/// The Arabic the server chose for [undoDeliveryTo], rendered as-is.
@override@JsonKey(name: 'undo_delivery_to_label') final  String? undoDeliveryToLabel;
...
/// never walks the وسيط road.
@override@JsonKey(name: 'manufacturing_started_at') final  DateTime? manufacturingStartedAt;
...
/// own Arabic. Null on every list row; see [StockEffect] for why it is built only for an
/// order somebody actually opened, and why not one word of it is written in Dart.
@override@JsonKey(name: 'stock_effect') final  StockEffect? stockEffect;
/// Who took the order. Null on the list — the server sends it with the full order only —
/// and for an order raised by a seeder or a console command.
///
/// Read by «ملاحظات الطلبية»: the order's own note is the one note with no transition
/// behind it, and this is the closest thing anybody recorded about who wrote it.
@override@JsonKey(name: 'created_by') final  OrderActor? createdBy;
...
@override
bool operator ==(Object other) {
  return identical(this, other) || (other.runtimeType == runtimeType&&other is _Order&&(identical(other.id, id) || other.id == id)&&(identical(other.code, code) || other.code == code)&&(identical(other.status, status) || other.status == status)&&(identical(other.statusLabel, statusLabel) || other.statusLabel == statusLabel)&&(identical(other.productionFlowLabel, productionFlowLabel) || other.productionFlowLabel == productionFlowLabel)&&(identical(other.isFinal, isFinal) || other.isFinal == isFinal)&&(identical(other.isClosed, isClosed) || other.isClosed == isClosed)&&(identical(other.isUrgent, isUrgent) || other.isUrgent == isUrgent)&&(identical(other.readyMessageApplies, readyMessageApplies) || other.readyMessageApplies == readyMessageApplies)&&(identical(other.isReadyMessageSent, isReadyMessageSent) || other.isReadyMessageSent == isReadyMessageSent)&&(identical(other.readyMessageSentAt, readyMessageSentAt) || other.readyMessageSentAt == readyMessageSentAt)&&(identical(other.readyMessageSentBy, readyMessageSentBy) || other.readyMessageSentBy == readyMessageSentBy)&&(identical(other.depositExpectedAmount, depositExpectedAmount) || other.depositExpectedAmount == depositExpectedAmount)&&(identical(other.depositExpectedMethod, depositExpectedMethod) || other.depositExpectedMethod == depositExpectedMethod)&&(identical(other.depositExpectedMethodLabel, depositExpectedMethodLabel) || other.depositExpectedMethodLabel == depositExpectedMethodLabel)&&(identical(other.depositPaidAt, depositPaidAt) || other.depositPaidAt == depositPaidAt)&&(identical(other.isDepositReceived, isDepositReceived) || other.isDepositReceived == isDepositReceived)&&(identical(other.depositConfirmedAt, depositConfirmedAt) || other.depositConfirmedAt == depositConfirmedAt)&&(identical(other.canConfirmDeposit, canConfirmDeposit) || other.canConfirmDeposit == canConfirmDeposit)&&(identical(other.awaitsDepositConfirmation, awaitsDepositConfirmation) || other.awaitsDepositConfirmation == awaitsDepositConfirmation)&&(identical(other.depositClaimedBy, depositClaimedBy) || other.depositClaimedBy == depositClaimedBy)&&(identical(other.depositConfirmedBy, depositConfirmedBy) || other.depositConfirmedBy == depositConfirmedBy)&&(identical(other.isPartiallyDelivered, isPartiallyDelivered) || other.isPartiallyDelivered == isPartiallyDelivered)&&const DeepCollectionEquality().equals(other._availableTransitions, _availableTransitions)&&(identical(other.customerId, customerId) || other.customerId == customerId)&&(identical(other.cityId, cityId) || other.cityId == cityId)&&(identical(other.designSource, designSource) || other.designSource == designSource)&&(identical(other.cityName, cityName) || other.cityName == cityName)&&(identical(other.fulfilmentTypeLabel, fulfilmentTypeLabel) || other.fulfilmentTypeLabel == fulfilmentTypeLabel)&&(identical(other.isOfficePickup, isOfficePickup) || other.isOfficePickup == isOfficePickup)&&(identical(other.designSourceLabel, designSourceLabel) || other.designSourceLabel == designSourceLabel)&&(identical(other.itemsTotal, itemsTotal) || other.itemsTotal == itemsTotal)&&(identical(other.designFee, designFee) || other.designFee == designFee)&&(identical(other.deliveryPrice, deliveryPrice) || other.deliveryPrice == deliveryPrice)&&(identical(other.discount, discount) || other.discount == discount)&&(identical(other.additionalCost, additionalCost) || other.additionalCost == additionalCost)&&(identical(other.additionalCostReason, additionalCostReason) || other.additionalCostReason == additionalCostReason)&&(identical(other.additionalCostReasonLabel, additionalCostReasonLabel) || other.additionalCostReasonLabel == additionalCostReasonLabel)&&(identical(other.additionalCostNote, additionalCostNote) || other.additionalCostNote == additionalCostNote)&&(identical(other.grandTotal, grandTotal) || other.grandTotal == grandTotal)&&(identical(other.paidAmount, paidAmount) || other.paidAmount == paidAmount)&&(identical(other.writtenOffAmount, writtenOffAmount) || other.writtenOffAmount == writtenOffAmount)&&(identical(other.carrierSettledAmount, carrierSettledAmount) || other.carrierSettledAmount == carrierSettledAmount)&&(identical(other.remainingAmount, remainingAmount) || other.remainingAmount == remainingAmount)&&(identical(other.paymentStatus, paymentStatus) || other.paymentStatus == paymentStatus)&&(identical(other.paymentStatusLabel, paymentStatusLabel) || other.paymentStatusLabel == paymentStatusLabel)&&(identical(other.hasUnrecordedMoney, hasUnrecordedMoney) || other.hasUnrecordedMoney == hasUnrecordedMoney)&&(identical(other.collectedAmount, collectedAmount) || other.collectedAmount == collectedAmount)&&(identical(other.customer, customer) || other.customer == customer)&&(identical(other.regionId, regionId) || other.regionId == regionId)&&(identical(other.customerShopId, customerShopId) || other.customerShopId == customerShopId)&&(identical(other.regionName, regionName) || other.regionName == regionName)&&(identical(other.customerShopName, customerShopName) || other.customerShopName == customerShopName)&&(identical(other.recipientName, recipientName) || other.recipientName == recipientName)&&(identical(other.recipientPhone, recipientPhone) || other.recipientPhone == recipientPhone)&&(identical(other.addressDetails, addressDetails) || other.addressDetails == addressDetails)&&(identical(other.notes, notes) || other.notes == notes)&&(identical(other.shippingCompany, shippingCompany) || other.shippingCompany == shippingCompany)&&(identical(other.trackingNumber, trackingNumber) || other.trackingNumber == trackingNumber)&&(identical(other.nawrisParcel, nawrisParcel) || other.nawrisParcel == nawrisParcel)&&(identical(other.courierPhone, courierPhone) || other.courierPhone == courierPhone)&&(identical(other.cancellationReason, cancellationReason) || other.cancellationReason == cancellationReason)&&(identical(other.reinstateTo, reinstateTo) || other.reinstateTo == reinstateTo)&&(identical(other.reinstateToLabel, reinstateToLabel) || other.reinstateToLabel == reinstateToLabel)&&(identical(other.canUnsettle, canUnsettle) || other.canUnsettle == canUnsettle)&&(identical(other.undoDeliveryTo, undoDeliveryTo) || other.undoDeliveryTo == undoDeliveryTo)&&(identical(other.undoDeliveryToLabel, undoDeliveryToLabel) || other.undoDeliveryToLabel == undoDeliveryToLabel)&&(identical(other.progress, progress) || other.progress == progress)&&(identical(other.itemsAreEditable, itemsAreEditable) || other.itemsAreEditable == itemsAreEditable)&&(identical(other.designsAreEditable, designsAreEditable) || other.designsAreEditable == designsAreEditable)&&(identical(other.destinationIsEditable, destinationIsEditable) || other.destinationIsEditable == destinationIsEditable)&&(identical(other.itemsCount, itemsCount) || other.itemsCount == itemsCount)&&const DeepCollectionEquality().equals(other._items, _items)&&const DeepCollectionEquality().equals(other._designs, _designs)&&const DeepCollectionEquality().equals(other._transitions, _transitions)&&(identical(other.totalCogs, totalCogs) || other.totalCogs == totalCogs)&&(identical(other.grossProfit, grossProfit) || other.grossProfit == grossProfit)&&(identical(other.fulfillmentWarehouseId, fulfillmentWarehouseId) || other.fulfillmentWarehouseId == fulfillmentWarehouseId)&&(identical(other.stockDeductedAt, stockDeductedAt) || other.stockDeductedAt == stockDeductedAt)&&(identical(other.vendorId, vendorId) || other.vendorId == vendorId)&&(identical(other.vendorName, vendorName) || other.vendorName == vendorName)&&(identical(other.manufacturingStartedAt, manufacturingStartedAt) || other.manufacturingStartedAt == manufacturingStartedAt)&&(identical(other.totalWeight, totalWeight) || other.totalWeight == totalWeight)&&(identical(other.placedAt, placedAt) || other.placedAt == placedAt)&&(identical(other.deliveredAt, deliveredAt) || other.deliveredAt == deliveredAt)&&(identical(other.settledAt, settledAt) || other.settledAt == settledAt)&&(identical(other.createdAt, createdAt) || other.createdAt == createdAt)&&(identical(other.deletedAt, deletedAt) || other.deletedAt == deletedAt)&&(identical(other.stockEffect, stockEffect) || other.stockEffect == stockEffect)&&(identical(other.createdBy, createdBy) || other.createdBy == createdBy));
}

@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,code,status,statusLabel,productionFlowLabel,isFinal,isClosed,isUrgent,readyMessageApplies,isReadyMessageSent,readyMessageSentAt,readyMessageSentBy,depositExpectedAmount,depositExpectedMethod,depositExpectedMethodLabel,depositPaidAt,isDepositReceived,depositConfirmedAt,canConfirmDeposit,awaitsDepositConfirmation,depositClaimedBy,depositConfirmedBy,isPartiallyDelivered,const DeepCollectionEquality().hash(_availableTransitions),customerId,cityId,designSource,cityName,fulfilmentTypeLabel,isOfficePickup,designSourceLabel,itemsTotal,designFee,deliveryPrice,discount,additionalCost,additionalCostReason,additionalCostReasonLabel,additionalCostNote,grandTotal,paidAmount,writtenOffAmount,carrierSettledAmount,remainingAmount,paymentStatus,paymentStatusLabel,hasUnrecordedMoney,collectedAmount,customer,regionId,customerShopId,regionName,customerShopName,recipientName,recipientPhone,addressDetails,notes,shippingCompany,trackingNumber,nawrisParcel,courierPhone,cancellationReason,reinstateTo,reinstateToLabel,canUnsettle,undoDeliveryTo,undoDeliveryToLabel,progress,itemsAreEditable,designsAreEditable,destinationIsEditable,itemsCount,const DeepCollectionEquality().hash(_items),const DeepCollectionEquality().hash(_designs),const DeepCollectionEquality().hash(_transitions),totalCogs,grossProfit,fulfillmentWarehouseId,stockDeductedAt,vendorId,vendorName,manufacturingStartedAt,totalWeight,placedAt,deliveredAt,settledAt,createdAt,deletedAt,stockEffect,createdBy]);
...
@override $OrderActorCopyWith<$Res>? get readyMessageSentBy;@override $OrderActorCopyWith<$Res>? get depositClaimedBy;@override $OrderActorCopyWith<$Res>? get depositConfirmedBy;@override $CustomerCopyWith<$Res>? get customer;@override $NawrisParcelRefCopyWith<$Res>? get nawrisParcel;@override $OrderProgressCopyWith<$Res> get progress;@override $StockEffectCopyWith<$Res>? get stockEffect;@override $OrderActorCopyWith<$Res>? get createdBy;
...
/// Create a copy of Order
/// with the given fields replaced by the non-null parameter values.
@override @pragma('vm:prefer-inline') $Res call({Object? id = null,Object? code = null,Object? status = null,Object? statusLabel = null,Object? productionFlowLabel = null,Object? isFinal = null,Object? isClosed = null,Object? isUrgent = null,Object? readyMessageApplies = null,Object? isReadyMessageSent = null,Object? readyMessageSentAt = freezed,Object? readyMessageSentBy = freezed,Object? depositExpectedAmount = freezed,Object? depositExpectedMethod = freezed,Object? depositExpectedMethodLabel = freezed,Object? depositPaidAt = freezed,Object? isDepositReceived = null,Object? depositConfirmedAt = freezed,Object? canConfirmDeposit = null,Object? awaitsDepositConfirmation = null,Object? depositClaimedBy = freezed,Object? depositConfirmedBy = freezed,Object? isPartiallyDelivered = freezed,Object? availableTransitions = null,Object? customerId = null,Object? cityId = null,Object? designSource = null,Object? cityName = null,Object? fulfilmentTypeLabel = null,Object? isOfficePickup = null,Object? designSourceLabel = null,Object? itemsTotal = null,Object? designFee = null,Object? deliveryPrice = null,Object? discount = null,Object? additionalCost = null,Object? additionalCostReason = freezed,Object? additionalCostReasonLabel = freezed,Object? additionalCostNote = freezed,Object? grandTotal = null,Object? paidAmount = null,Object? writtenOffAmount = null,Object? carrierSettledAmount = null,Object? remainingAmount = null,Object? paymentStatus = null,Object? paymentStatusLabel = null,Object? hasUnrecordedMoney = null,Object? collectedAmount = freezed,Object? customer = freezed,Object? regionId = freezed,Object? customerShopId = freezed,Object? regionName = freezed,Object? customerShopName = freezed,Object? recipientName = freezed,Object? recipientPhone = freezed,Object? addressDetails = freezed,Object? notes = freezed,Object? shippingCompany = freezed,Object? trackingNumber = freezed,Object? nawrisParcel = freezed,Object? courierPhone = freezed,Object? cancellationReason = freezed,Object? reinstateTo = freezed,Object? reinstateToLabel = freezed,Object? canUnsettle = null,Object? undoDeliveryTo = freezed,Object? undoDeliveryToLabel = freezed,Object? progress = null,Object? itemsAreEditable = null,Object? designsAreEditable = null,Object? destinationIsEditable = null,Object? itemsCount = freezed,Object? items = freezed,Object? designs = freezed,Object? transitions = freezed,Object? totalCogs = freezed,Object? grossProfit = freezed,Object? fulfillmentWarehouseId = freezed,Object? stockDeductedAt = freezed,Object? vendorId = freezed,Object? vendorName = freezed,Object? manufacturingStartedAt = freezed,Object? totalWeight = freezed,Object? placedAt = freezed,Object? deliveredAt = freezed,Object? settledAt = freezed,Object? createdAt = freezed,Object? deletedAt = freezed,Object? stockEffect = freezed,Object? createdBy = freezed,}) {
  return _then(_Order(
id: null == id ? _self.id : id // ignore: cast_nullable_to_non_nullable
as int,code: null == code ? _self.code : code // ignore: cast_nullable_to_non_nullable
as String,status: null == status ? _self.status : status // ignore: cast_nullable_to_non_nullable
...
@JsonKey(includeFromJson: false, includeToJson: false)
@pragma('vm:prefer-inline')
$NawrisParcelRefCopyWith<NawrisParcelRef> get copyWith => _$NawrisParcelRefCopyWithImpl<NawrisParcelRef>(this as NawrisParcelRef, _$identity);

  /// Serializes this NawrisParcelRef to a JSON map.
  Map<String, dynamic> toJson();
...
/// @nodoc
abstract mixin class $NawrisParcelRefCopyWith<$Res>  {
...
/// @nodoc
class _$NawrisParcelRefCopyWithImpl<$Res>
    implements $NawrisParcelRefCopyWith<$Res> {
  _$NawrisParcelRefCopyWithImpl(this._self, this._then);

  final NawrisParcelRef _self;
  final $Res Function(NawrisParcelRef) _then;
...
/// Adds pattern-matching-related methods to [NawrisParcelRef].
extension NawrisParcelRefPatterns on NawrisParcelRef {
/// A variant of `map` that fallback to returning `orElse`.
///
/// It is equivalent to doing:
/// ```dart
...
@optionalTypeArgs TResult maybeMap<TResult extends Object?>(TResult Function( _NawrisParcelRef value)?  $default,{required TResult orElse(),}){
final _that = this;
switch (_that) {
...
@optionalTypeArgs TResult map<TResult extends Object?>(TResult Function( _NawrisParcelRef value)  $default,){
final _that = this;
switch (_that) {
...
@optionalTypeArgs TResult? mapOrNull<TResult extends Object?>(TResult? Function( _NawrisParcelRef value)?  $default,){
final _that = this;
switch (_that) {
case _NawrisParcelRef() when $default != null:
return $default(_that);case _:
...
@override @JsonKey(includeFromJson: false, includeToJson: false)
@pragma('vm:prefer-inline')
_$NawrisParcelRefCopyWith<_NawrisParcelRef> get copyWith => __$NawrisParcelRefCopyWithImpl<_NawrisParcelRef>(this, _$identity);

@override
Map<String, dynamic> toJson() {
  return _$NawrisParcelRefToJson(this, );
}
...
/// @nodoc
abstract mixin class _$NawrisParcelRefCopyWith<$Res> implements $NawrisParcelRefCopyWith<$Res> {
...
/// @nodoc
class __$NawrisParcelRefCopyWithImpl<$Res>
    implements _$NawrisParcelRefCopyWith<$Res> {
  __$NawrisParcelRefCopyWithImpl(this._self, this._then);

  final _NawrisParcelRef _self;
  final $Res Function(_NawrisParcelRef) _then;
...
@JsonKey(name: 'shortage_quantity') String? get shortageQuantity;/// The same gap in the unit the **warehouse** counts — what «النواقص» chases and buys.
///
...
@JsonKey(name: 'shortage_warehouse_quantity') String? get shortageWarehouseQuantity;/// What the customer left on the counter, in this line's own unit.
///
...
/// above already makes.
@JsonKey(name: 'undelivered_quantity') String? get undeliveredQuantity;/// What became of it: `restocked` or `written_off`. Read off the line rather than worked
/// out from the product's heading — re-filing a product must not rewrite a delivery
/// recorded last March.
@JsonKey(name: 'undelivered_disposition') String? get undeliveredDisposition;/// The same thing in Arabic, ready to print — «أُعيد إلى المخزن» or «خسارة». Sent beside
/// the value for the reason [pricingUnitLabel] is: the app branches on one and shows the
/// other, and never owns the dictionary.
@JsonKey(name: 'undelivered_disposition_label') String? get undeliveredDispositionLabel;/// كم من المتروك وصل الرفّ فعلاً — **بوحدة المخزن**، لا بوحدة البيع التي عُدَّ بها
...
/// أحداً فتح الرفّ.
@JsonKey(name: 'restocked_quantity') String? get restockedQuantity;/// What the goods left behind cost us — **null unless they were a loss**. Bags back on the
/// shelf cost the shop nothing, so a restocked line has no figure here at all.
@JsonKey(name: 'delivery_loss') String? get deliveryLoss;/// What the line is actually charged for: [quantity] less [shortageQuantity], less
/// [undeliveredQuantity].
///
...
@JsonKey(name: 'material_cost') String? get materialCost;/// What that material actually cost the business, when the two are not the same number.
///
...
@JsonKey(name: 'material_cost_actual') String? get materialCostActual;/// When the press bought this line's plain material off the shelf — and null on every line
/// that did not.
///
...
@JsonKey(name: 'stock_purchased_at') String? get stockPurchasedAt;@JsonKey(name: 'labor_cost') String? get laborCost;@JsonKey(name: 'overhead_cost') String? get overheadCost;/// The three above, added up by the server. Read rather than summed here for the same reason
/// [billableQuantity] is.
...
@JsonKey(name: 'stock_unit_label') String? get stockUnitLabel;/// Whether this line's shortage has to be stated twice — once for the invoice and once for
/// the shelf.
///
...
@JsonKey(name: 'is_stocked_in_another_unit') bool? get isStockedInAnotherUnit;/// The copy of the size's «سعر التكلفة» taken the day this order was made — what makes a
...
@JsonKey(name: 'unit_cost') String? get unitCost;/// What the line cost in total, written when the vendor handed the job over («جاهزة»). Null
...
@JsonKey(name: 'outsourcing_cost') String? get outsourcingCost; String? get notes;
...
@override
bool operator ==(Object other) {
  return identical(this, other) || (other.runtimeType == runtimeType&&other is OrderItem&&(identical(other.id, id) || other.id == id)&&(identical(other.productId, productId) || other.productId == productId)&&(identical(other.productVariantId, productVariantId) || other.productVariantId == productVariantId)&&(identical(other.productName, productName) || other.productName == productName)&&(identical(other.variantLabel, variantLabel) || other.variantLabel == variantLabel)&&(identical(other.productCode, productCode) || other.productCode == productCode)&&(identical(other.productImage, productImage) || other.productImage == productImage)&&(identical(other.pricingUnitLabel, pricingUnitLabel) || other.pricingUnitLabel == pricingUnitLabel)&&(identical(other.quantity, quantity) || other.quantity == quantity)&&(identical(other.shortageQuantity, shortageQuantity) || other.shortageQuantity == shortageQuantity)&&(identical(other.shortageWarehouseQuantity, shortageWarehouseQuantity) || other.shortageWarehouseQuantity == shortageWarehouseQuantity)&&(identical(other.undeliveredQuantity, undeliveredQuantity) || other.undeliveredQuantity == undeliveredQuantity)&&(identical(other.undeliveredDisposition, undeliveredDisposition) || other.undeliveredDisposition == undeliveredDisposition)&&(identical(other.undeliveredDispositionLabel, undeliveredDispositionLabel) || other.undeliveredDispositionLabel == undeliveredDispositionLabel)&&(identical(other.restockedQuantity, restockedQuantity) || other.restockedQuantity == restockedQuantity)&&(identical(other.deliveryLoss, deliveryLoss) || other.deliveryLoss == deliveryLoss)&&(identical(other.billableQuantity, billableQuantity) || other.billableQuantity == billableQuantity)&&(identical(other.warehouseQuantity, warehouseQuantity) || other.warehouseQuantity == warehouseQuantity)&&(identical(other.unitPrice, unitPrice) || other.unitPrice == unitPrice)&&(identical(other.lineTotal, lineTotal) || other.lineTotal == lineTotal)&&(identical(other.materialCost, materialCost) || other.materialCost == materialCost)&&(identical(other.materialCostActual, materialCostActual) || other.materialCostActual == materialCostActual)&&(identical(other.stockPurchasedAt, stockPurchasedAt) || other.stockPurchasedAt == stockPurchasedAt)&&(identical(other.laborCost, laborCost) || other.laborCost == laborCost)&&(identical(other.overheadCost, overheadCost) || other.overheadCost == overheadCost)&&(identical(other.cogs, cogs) || other.cogs == cogs)&&(identical(other.unitMaterialCost, unitMaterialCost) || other.unitMaterialCost == unitMaterialCost)&&(identical(other.stockUnitLabel, stockUnitLabel) || other.stockUnitLabel == stockUnitLabel)&&(identical(other.isStockedInAnotherUnit, isStockedInAnotherUnit) || other.isStockedInAnotherUnit == isStockedInAnotherUnit)&&(identical(other.unitCost, unitCost) || other.unitCost == unitCost)&&(identical(other.outsourcingCost, outsourcingCost) || other.outsourcingCost == outsourcingCost)&&(identical(other.notes, notes) || other.notes == notes));
}

@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,productId,productVariantId,productName,variantLabel,productCode,productImage,pricingUnitLabel,quantity,shortageQuantity,shortageWarehouseQuantity,undeliveredQuantity,undeliveredDisposition,undeliveredDispositionLabel,restockedQuantity,deliveryLoss,billableQuantity,warehouseQuantity,unitPrice,lineTotal,materialCost,materialCostActual,stockPurchasedAt,laborCost,overheadCost,cogs,unitMaterialCost,stockUnitLabel,isStockedInAnotherUnit,unitCost,outsourcingCost,notes]);

@override
String toString() {
  return 'OrderItem(id: $id, productId: $productId, productVariantId: $productVariantId, productName: $productName, variantLabel: $variantLabel, productCode: $productCode, productImage: $productImage, pricingUnitLabel: $pricingUnitLabel, quantity: $quantity, shortageQuantity: $shortageQuantity, shortageWarehouseQuantity: $shortageWarehouseQuantity, undeliveredQuantity: $undeliveredQuantity, undeliveredDisposition: $undeliveredDisposition, undeliveredDispositionLabel: $undeliveredDispositionLabel, restockedQuantity: $restockedQuantity, deliveryLoss: $deliveryLoss, billableQuantity: $billableQuantity, warehouseQuantity: $warehouseQuantity, unitPrice: $unitPrice, lineTotal: $lineTotal, materialCost: $materialCost, materialCostActual: $materialCostActual, stockPurchasedAt: $stockPurchasedAt, laborCost: $laborCost, overheadCost: $overheadCost, cogs: $cogs, unitMaterialCost: $unitMaterialCost, stockUnitLabel: $stockUnitLabel, isStockedInAnotherUnit: $isStockedInAnotherUnit, unitCost: $unitCost, outsourcingCost: $outsourcingCost, notes: $notes)';
}
...
@useResult
$Res call({
 int id,@JsonKey(name: 'product_id') int productId,@JsonKey(name: 'product_variant_id') int productVariantId,@JsonKey(name: 'product_name') String productName,@JsonKey(name: 'variant_label') String variantLabel,@JsonKey(name: 'product_code') String? productCode,@JsonKey(name: 'product_image') ProductImage? productImage,@JsonKey(name: 'pricing_unit_label') String pricingUnitLabel, String quantity,@JsonKey(name: 'shortage_quantity') String? shortageQuantity,@JsonKey(name: 'shortage_warehouse_quantity') String? shortageWarehouseQuantity,@JsonKey(name: 'undelivered_quantity') String? undeliveredQuantity,@JsonKey(name: 'undelivered_disposition') String? undeliveredDisposition,@JsonKey(name: 'undelivered_disposition_label') String? undeliveredDispositionLabel,@JsonKey(name: 'restocked_quantity') String? restockedQuantity,@JsonKey(name: 'delivery_loss') String? deliveryLoss,@JsonKey(name: 'billable_quantity') String? billableQuantity,@JsonKey(name: 'warehouse_quantity') String? warehouseQuantity,@JsonKey(name: 'unit_price') String unitPrice,@JsonKey(name: 'line_total') String lineTotal,@JsonKey(name: 'material_cost') String? materialCost,@JsonKey(name: 'material_cost_actual') String? materialCostActual,@JsonKey(name: 'stock_purchased_at') String? stockPurchasedAt,@JsonKey(name: 'labor_cost') String? laborCost,@JsonKey(name: 'overhead_cost') String? overheadCost, String? cogs,@JsonKey(name: 'unit_material_cost') String? unitMaterialCost,@JsonKey(name: 'stock_unit_label') String? stockUnitLabel,@JsonKey(name: 'is_stocked_in_another_unit') bool? isStockedInAnotherUnit,@JsonKey(name: 'unit_cost') String? unitCost,@JsonKey(name: 'outsourcing_cost') String? outsourcingCost, String? notes
});
...
/// Create a copy of OrderItem
/// with the given fields replaced by the non-null parameter values.
@pragma('vm:prefer-inline') @override $Res call({Object? id = null,Object? productId = null,Object? productVariantId = null,Object? productName = null,Object? variantLabel = null,Object? productCode = freezed,Object? productImage = freezed,Object? pricingUnitLabel = null,Object? quantity = null,Object? shortageQuantity = freezed,Object? shortageWarehouseQuantity = freezed,Object? undeliveredQuantity = freezed,Object? undeliveredDisposition = freezed,Object? undeliveredDispositionLabel = freezed,Object? restockedQuantity = freezed,Object? deliveryLoss = freezed,Object? billableQuantity = freezed,Object? warehouseQuantity = freezed,Object? unitPrice = null,Object? lineTotal = null,Object? materialCost = freezed,Object? materialCostActual = freezed,Object? stockPurchasedAt = freezed,Object? laborCost = freezed,Object? overheadCost = freezed,Object? cogs = freezed,Object? unitMaterialCost = freezed,Object? stockUnitLabel = freezed,Object? isStockedInAnotherUnit = freezed,Object? unitCost = freezed,Object? outsourcingCost = freezed,Object? notes = freezed,}) {
  return _then(_self.copyWith(
id: null == id ? _self.id : id // ignore: cast_nullable_to_non_nullable
...
as String?,shortageWarehouseQuantity: freezed == shortageWarehouseQuantity ? _self.shortageWarehouseQuantity : shortageWarehouseQuantity // ignore: cast_nullable_to_non_nullable
as String?,undeliveredQuantity: freezed == undeliveredQuantity ? _self.undeliveredQuantity : undeliveredQuantity // ignore: cast_nullable_to_non_nullable
as String?,undeliveredDisposition: freezed == undeliveredDisposition ? _self.undeliveredDisposition : undeliveredDisposition // ignore: cast_nullable_to_non_nullable
as String?,undeliveredDispositionLabel: freezed == undeliveredDispositionLabel ? _self.undeliveredDispositionLabel : undeliveredDispositionLabel // ignore: cast_nullable_to_non_nullable
as String?,restockedQuantity: freezed == restockedQuantity ? _self.restockedQuantity : restockedQuantity // ignore: cast_nullable_to_non_nullable
as String?,deliveryLoss: freezed == deliveryLoss ? _self.deliveryLoss : deliveryLoss // ignore: cast_nullable_to_non_nullable
...
as String?,materialCostActual: freezed == materialCostActual ? _self.materialCostActual : materialCostActual // ignore: cast_nullable_to_non_nullable
as String?,stockPurchasedAt: freezed == stockPurchasedAt ? _self.stockPurchasedAt : stockPurchasedAt // ignore: cast_nullable_to_non_nullable
...
as String?,isStockedInAnotherUnit: freezed == isStockedInAnotherUnit ? _self.isStockedInAnotherUnit : isStockedInAnotherUnit // ignore: cast_nullable_to_non_nullable
as bool?,unitCost: freezed == unitCost ? _self.unitCost : unitCost // ignore: cast_nullable_to_non_nullable
as String?,outsourcingCost: freezed == outsourcingCost ? _self.outsourcingCost : outsourcingCost // ignore: cast_nullable_to_non_nullable
as String?,notes: freezed == notes ? _self.notes : notes // ignore: cast_nullable_to_non_nullable
as String?,
  ));
}
...
switch (_that) {
case _OrderItem() when $default != null:
return $default(_that.id,_that.productId,_that.productVariantId,_that.productName,_that.variantLabel,_that.productCode,_that.productImage,_that.pricingUnitLabel,_that.quantity,_that.shortageQuantity,_that.shortageWarehouseQuantity,_that.undeliveredQuantity,_that.undeliveredDisposition,_that.undeliveredDispositionLabel,_that.restockedQuantity,_that.deliveryLoss,_that.billableQuantity,_that.warehouseQuantity,_that.unitPrice,_that.lineTotal,_that.materialCost,_that.materialCostActual,_that.stockPurchasedAt,_that.laborCost,_that.overheadCost,_that.cogs,_that.unitMaterialCost,_that.stockUnitLabel,_that.isStockedInAnotherUnit,_that.unitCost,_that.outsourcingCost,_that.notes);case _:
  return orElse();
...
class _OrderItem extends OrderItem {
  const _OrderItem({required this.id, @JsonKey(name: 'product_id') required this.productId, @JsonKey(name: 'product_variant_id') required this.productVariantId, @JsonKey(name: 'product_name') required this.productName, @JsonKey(name: 'variant_label') required this.variantLabel, @JsonKey(name: 'product_code') this.productCode, @JsonKey(name: 'product_image') this.productImage, @JsonKey(name: 'pricing_unit_label') required this.pricingUnitLabel, required this.quantity, @JsonKey(name: 'shortage_quantity') this.shortageQuantity, @JsonKey(name: 'shortage_warehouse_quantity') this.shortageWarehouseQuantity, @JsonKey(name: 'undelivered_quantity') this.undeliveredQuantity, @JsonKey(name: 'undelivered_disposition') this.undeliveredDisposition, @JsonKey(name: 'undelivered_disposition_label') this.undeliveredDispositionLabel, @JsonKey(name: 'restocked_quantity') this.restockedQuantity, @JsonKey(name: 'delivery_loss') this.deliveryLoss, @JsonKey(name: 'billable_quantity') this.billableQuantity, @JsonKey(name: 'warehouse_quantity') this.warehouseQuantity, @JsonKey(name: 'unit_price') required this.unitPrice, @JsonKey(name: 'line_total') required this.lineTotal, @JsonKey(name: 'material_cost') this.materialCost, @JsonKey(name: 'material_cost_actual') this.materialCostActual, @JsonKey(name: 'stock_purchased_at') this.stockPurchasedAt, @JsonKey(name: 'labor_cost') this.laborCost, @JsonKey(name: 'overhead_cost') this.overheadCost, this.cogs, @JsonKey(name: 'unit_material_cost') this.unitMaterialCost, @JsonKey(name: 'stock_unit_label') this.stockUnitLabel, @JsonKey(name: 'is_stocked_in_another_unit') this.isStockedInAnotherUnit, @JsonKey(name: 'unit_cost') this.unitCost, @JsonKey(name: 'outsourcing_cost') this.outsourcingCost, this.notes}): super._();
...
///
/// Null where the two units agree, which is most lines, and means «the same number».
@override@JsonKey(name: 'shortage_warehouse_quantity') final  String? shortageWarehouseQuantity;
/// What the customer left on the counter, in this line's own unit.
///
/// Null on every line of every order delivered whole, which is nearly all of them —
/// «nothing recorded» rather than «nothing left», the distinction [shortageQuantity]
/// above already makes.
@override@JsonKey(name: 'undelivered_quantity') final  String? undeliveredQuantity;
/// What became of it: `restocked` or `written_off`. Read off the line rather than worked
/// out from the product's heading — re-filing a product must not rewrite a delivery
/// recorded last March.
@override@JsonKey(name: 'undelivered_disposition') final  String? undeliveredDisposition;
/// The same thing in Arabic, ready to print — «أُعيد إلى المخزن» or «خسارة». Sent beside
/// the value for the reason [pricingUnitLabel] is: the app branches on one and shows the
/// other, and never owns the dictionary.
@override@JsonKey(name: 'undelivered_disposition_label') final  String? undeliveredDispositionLabel;
...
/// خالٍ حيث لم يرجع شيء، والمطبوع منه: تلك أكياسٌ تحمل تصميم الزبون، وصفرٌ هنا ادّعاءٌ بأن
/// أحداً فتح الرفّ.
@override@JsonKey(name: 'restocked_quantity') final  String? restockedQuantity;
/// What the goods left behind cost us — **null unless they were a loss**. Bags back on the
/// shelf cost the shop nothing, so a restocked line has no figure here at all.
@override@JsonKey(name: 'delivery_loss') final  String? deliveryLoss;
...
/// own profit has to carry, and this is what the goods cost whoever financed them. Equal on
/// every line that bought nothing, which is most of them.
@override@JsonKey(name: 'material_cost_actual') final  String? materialCostActual;
...
/// delivery will not pay him again, and cancelling this order returns the bags to the
/// company rather than to his deal. «استلم الزبون ما استلمش، المطبعة تتحمّل.»
@override@JsonKey(name: 'stock_purchased_at') final  String? stockPurchasedAt;
...
/// rather than false where the shelf was not loaded, so «the units agree» is never read out
/// of a payload that simply never asked.
@override@JsonKey(name: 'is_stocked_in_another_unit') final  bool? isStockedInAnotherUnit;
...
/// before that: a price agreed with a vendor is not a cost incurred. Folded into [cogs] by
/// the server, so it is read here and never added in again.
@override@JsonKey(name: 'outsourcing_cost') final  String? outsourcingCost;
@override final  String? notes;
...
/// Create a copy of OrderItem
/// with the given fields replaced by the non-null parameter values.
@override @pragma('vm:prefer-inline') $Res call({Object? id = null,Object? productId = null,Object? productVariantId = null,Object? productName = null,Object? variantLabel = null,Object? productCode = freezed,Object? productImage = freezed,Object? pricingUnitLabel = null,Object? quantity = null,Object? shortageQuantity = freezed,Object? shortageWarehouseQuantity = freezed,Object? undeliveredQuantity = freezed,Object? undeliveredDisposition = freezed,Object? undeliveredDispositionLabel = freezed,Object? restockedQuantity = freezed,Object? deliveryLoss = freezed,Object? billableQuantity = freezed,Object? warehouseQuantity = freezed,Object? unitPrice = null,Object? lineTotal = null,Object? materialCost = freezed,Object? materialCostActual = freezed,Object? stockPurchasedAt = freezed,Object? laborCost = freezed,Object? overheadCost = freezed,Object? cogs = freezed,Object? unitMaterialCost = freezed,Object? stockUnitLabel = freezed,Object? isStockedInAnotherUnit = freezed,Object? unitCost = freezed,Object? outsourcingCost = freezed,Object? notes = freezed,}) {
