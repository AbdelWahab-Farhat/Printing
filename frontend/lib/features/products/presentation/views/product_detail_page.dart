...
/// Stateful for one reason: it remembers the product as it changed, so the catalogue behind is
/// handed the new row instead of made to re-read a page it already holds. That is screen
/// lifecycle, not business state — the Cubit owns the product itself.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<ProductDetailCubit, ProductDetailState>(
        ...
        listener: (context, state) => context.handBack(_changes.saw(state.product)),
