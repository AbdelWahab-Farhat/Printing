...
///
/// A screen hands it a list and nothing else:
///
///   * **empty** → nothing is rendered. An empty dial is furniture.
///   * **one action** → a plain extended button carrying that action's label. A dial that opens
///     to reveal a single choice is a tax on every use of it.
///   * **more than one** → the dial.
///
/// > **There is no permission filter here, unlike the staff app.** A customer's account has no
/// > role matrix — what he may do is what his own records allow — so a screen that must hide an
/// > action hides it by not passing it. The day the API grants the client app abilities per
/// > account, the filter belongs here and nowhere else, exactly as it does over there.
///
...
class AppSpeedDial extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (actions.length == 1) return _Single(action: actions.single);

    return _Dial(actions: actions);
  }
}
