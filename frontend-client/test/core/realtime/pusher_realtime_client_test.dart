...
void main() {
  ...
  test('the first listener connects and enters the channel once the socket has an id', () async {
    ...
    // Act
    final subscription = client.privateEvents('private-customers.3', 'support.ticket.changed').listen((_) {});
    await settle();

    // Assert
    expect(server.connections, hasLength(1));
    expect(signed, [('1.1', 'private-customers.3')]);
    expect(server.subscribedTo('private-customers.3'), isTrue);
  ...
  test('an event on the channel arrives decoded, and other events and channels do not', () async {
    ...
    final subscription = client.privateEvents('private-customers.3', 'support.ticket.changed').listen(received.add);
    await settle();

    // Act
    server
      ..push('private-customers.3', 'support.ticket.changed', {'ticket': {'id': 7}})
      ..push('private-customers.3', 'something.else', {'ticket': {'id': 8}})
      ..push('private-support.desk', 'support.ticket.changed', {'ticket': {'id': 9}});
    await settle();
  ...
  test('the last listener leaving closes the socket; the next one opens a new one', () async {
    // Arrange
    final client = build();
    final first = client.privateEvents('private-customers.3', 'support.ticket.changed').listen((_) {});
    final second = client.privateEvents('private-customers.3', 'support.ticket.changed').listen((_) {});
    await settle();
    ...
    final again = client.privateEvents('private-customers.3', 'support.ticket.changed').listen((_) {});
    await settle();
    ...
    expect(server.subscribedTo('private-customers.3', connection: 1), isTrue);
  ...
  test('after the server drops the socket the channel is entered again and resumed fires', () async {
    ...
    server.dropCurrent();
    await settle();
    server.push('private-customers.3', 'support.ticket.changed', {'ticket': {'id': 11}});
    await settle();
  ...
  test('a refused signature never subscribes', () async {
    ...
    // Assert
    expect(server.subscribedTo('private-customers.3'), isFalse);
  ...
  test('with no endpoint configured nothing connects and the stream stays silent', () async {
    ...
    final subscription = client
        .privateEvents('private-customers.3', 'support.ticket.changed')
