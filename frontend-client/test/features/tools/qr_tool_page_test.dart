...
void main() {
  ...
  testWidgets('يفتح بلا رمز', (tester) async {
    ...
    expect(find.text('أضف إلى تصاميمي'), findsNothing);
  });
  ...
  testWidgets('«إنشاء الرمز» يرسم ما كُتب، ويُظهر إجراءَي الرمز', (tester) async {
    // Arrange
    await tester.pumpWidget(host());
    await tester.pumpAndSettle();
    ...
    expect(find.text('أضف إلى تصاميمي'), findsOneWidget);
  });
  ...
  /// **هنا يفترق تطبيقُ العميل عن تطبيق الموظفين.**
  ///
  /// هناك: الشاشة المفتوحة من الدرج لا تعرض «إضافة إلى التصاميم» لأن لا مكان يذهب إليه الملف —
  /// مكتبةُ التصاميم تخصّ عميلاً يختاره الموظف، ولا عميلَ في السياق.
  ///
  /// وهنا: المكتبةُ مكتبةُ من يحمل الهاتف. فالزرّ حاضرٌ دائماً ويرفع بنفسه — انظر
  /// `_SaveToDesignsButton`. و«إضافة إلى التصاميم» (التي تُعيد ملفاً) تبقى لوضع الالتقاط وحده.
  testWidgets('حين تُفتح من «الأدوات» تعرض الحفظ في المكتبة بجانب التحميل', (tester) async {
    // Arrange
    await tester.pumpWidget(host());
    await tester.pumpAndSettle();
    ...
    expect(find.text('أضف إلى تصاميمي'), findsOneWidget);
    expect(find.text('تحميل'), findsOneWidget);
    // وليست تلك التي تُعيد ملفاً لمن فتح الشاشة — تلك لوضع الالتقاط.
    expect(find.text('إضافة إلى التصاميم'), findsNothing);
  });
