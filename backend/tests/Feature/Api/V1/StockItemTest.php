...
class StockItemTest extends TestCase
{
    ...
    public function test_the_list_can_be_searched_by_name(): void
    {
        ...
        // Act
        $response = $this->withHeaders($headers)->getJson('/api/v1/stock-items?search=ورقي');
    ...
    {
        // Arrange — declares the real physical unit; it does not convert the quantities, which
        // were correct in their own unit before and stay correct after
        $item = StockItem::factory()->create();
        ...
        $this->assertDatabaseHas('warehouse_stocks', [
            'stock_item_id' => $item->id, 'unit' => 'kilogram', 'quantity' => '40.000',
        ]);
