...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | صورةٌ أو PDF في رسالةٍ من أيّ الطرفين — صورةُ طلبيةٍ وصلت تالفة، أو التصميمُ
    | الذي يريده العميل، أو عرضُ سعرٍ يرسله المحل.
    |
    | **قرصٌ خاص كالتصاميم والإيصالات.** ما يُرسل في محادثة دعمٍ قد يكون فاتورةً
    | فيها رقمُ حساب، أو ملفَّ طباعةٍ هو ملكُ صاحبه؛ والقرص العام يوزّع روابط دائمة
    | بلا توقيع. `local` هو storage/app/private، ويُقدَّم برابطٍ موقَّع ينتهي.
    |
    | **الحدّ نفسه الذي للتصاميم**، لأن التصميم نفسه قد يُرسل من هنا. وكل حدٍّ هنا
    | يقابله فحصٌ مسبق في التطبيق، فرقمٌ ثانٍ مكانٌ ثالث يجب أن يتفق.
    |
    | `svg` غائبٌ ويبقى غائباً: SVG مستندُ HTML، ومستندٌ يُقدَّم من نطاقنا XSS مخزَّن.
    |
    */

    'ticket_attachments' => [
        'disk' => env('MEDIA_TICKET_ATTACHMENTS_DISK', 'local'),
        'max_kilobytes' => (int) env('MEDIA_TICKET_ATTACHMENT_MAX_KILOBYTES', 25600),
        'mimes' => ['pdf', 'jpg', 'jpeg', 'png', 'webp'],
        'mimetypes' => ['application/pdf', 'image/jpeg', 'image/png', 'image/webp'],
    ],
