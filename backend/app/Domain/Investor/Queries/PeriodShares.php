 ...
 *
 * ```
 * نصيبُ فلان في الفترة P = وحداتُه يومَ أُغلقت نافذةُ اكتتاب P ÷ مجموع الوحدات ذلك اليوم
 * ```
 *
 * ## لماذا يومَ أُغلقت النافذة، لا اليوم
 *
 * قرارُ المالك: «المالُ الداخل في منتصف الفترة … لا يحسب كربح ولا يحسب كنسبة الا بداية شهر
 * الجاي»، و«يمكنه فقط في بداية الفترة اول اسبوع او اول يوم مدة يحددها المدير». فالبابُ مفتوحٌ
 * في أول الفترة ثم يُغلق، **والصورةُ تُلتقط لحظةَ إغلاقه**: من دخل داخلها يقاسم هذه الفترة، ومن
 * تأخّر مالُه في الصندوق ونسبتُه من التالية.
 *
 * ولو قُرئت «اليوم» لتغيّرت قسمةُ شهرٍ كلَّما دخل داخل، وصار ربحُ طلبيةٍ من أوّله يُقسَّم بنسب
 * آخره.
 *
...
final class PeriodShares
{
    ...
    {
        return $this->units->byInvestor($period->subscription_closes_on->endOfDay());
    }
