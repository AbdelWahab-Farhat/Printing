 ...
 * ## ومن أين تبدأ
 *
 * **من الغد التالي لنهاية سابقتها، لا من اليوم.** فترةُ سبتمبر قد تُقفَل في ١٢ أكتوبر لأنها
 * تنتظر آخرَ طلبياتها؛ ولو بدأت التاليةُ يومَ الإقفال لسقط من الزمن أحدَ عشر يوماً لا تخصّ فترةً
 * أصلاً، وطلبياتُها لا تجد أين تقع. وأوّلُ فترةٍ في عمر الصندوق تبدأ اليوم لأن لا شيء قبلها.
 *
...
final class OpenInvestmentPeriod
{
    ...
    public function __invoke(?int $actorId): InvestmentPeriod
    {
        return DB::transaction(function () use ($actorId): InvestmentPeriod {
            ...
            // آخرُ يومٍ فيها، لا أوّلُ يومٍ بعدها: «شهرٌ» من الأول إلى الثلاثين، لا إلى الأول.
            $endsOn = $startsOn->copy()->addMonths($durations['period_months'])->subDay();

            // النافذةُ تشمل يومَ البدء، فسبعةُ أيامٍ من الأول تنتهي في السابع لا في الثامن.
            $subscriptionCloses = $startsOn->copy()
                ->addDays($durations['subscription_window_days'] - 1);
