...
described in the design doc (editable-only-while-new, the status map).
...
- **422 handling** — the "variant not on this order" failure comes back with `errors.items` (a
  ...
  the line list, not attached to one specific row, since the server doesn't say which row.
  **A quantity larger than what remains is not one of these failures**: the server books in
  whatever arrived, so the receive form must not cap its inputs at `quantityRemaining` either.
...
8. Cubit tests — fake `PurchaseOrderRepository`, assert the state sequence for each action
   including the 422 path (receiving an unordered variant, editing a non-`new` order).
